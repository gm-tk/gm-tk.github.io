...
describe('ModuleResultsPage — rewritten next-steps content', function () {
    ...
    it('upload step renders a nested lettered (type="a") sub-list of exactly three items', function () {
        ...
        assertEqual(step4.split('<li>').length - 1, 3, 'exactly three nested (a/b/c) sub-items');
    });

    it('upload sub-list names the three exact files in (a) Writer, (b) Media, (c) reference order', function () {
        var step4 = mdnsSteps()[3];
        var aPos = step4.indexOf('the Writer');
        var bPos = step4.indexOf('the Media List');
        var cPos = step4.indexOf('HTML files of an example module as a reference');
        assertTrue(aPos !== -1 && bPos !== -1 && cPos !== -1, 'all three sub-items are present');
        assertTrue(aPos < bPos && bPos < cPos, 'sub-items appear in (a) Writer, (b) Media, (c) reference order');
    });
