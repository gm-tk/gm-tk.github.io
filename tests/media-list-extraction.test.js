...
describe('MediaListConverter — structural exclusion of boilerplate', function () {
    ...
    it('parses a media list whose page-number column is "Pg No." (corpus majority), not "WTPg No."', function () {
        var headers = ['Item No.', 'Pg No.', 'Item Type', 'Description', 'Source', 'URL'];
        var out = convertExt([
            extMediaTable([['1', '5', 'Image', 'Hero image', 'iStock', 'https://ex.com/a.jpg']], headers)
        ]);
        assert(out.indexOf('Hero image') !== -1, 'recognised despite the "Pg No." label (not "WTPg No.")');
        assert(out.indexOf('Pg No.') !== -1, 'the table\'s actual header labels are rendered');
    });

    it('parses a media list with NO page-number column at all (the CEDO501 shape)', function () {
        var headers = ['Item No.', 'Item Type', 'Description', 'Source', 'URL']; // no Pg No./WTPg No.
        var table = extMediaTable([['1', 'Image', 'Income and Savings', 'iStock', 'https://ex.com/a.jpg']], headers);
        var out = convertExt([table]);
        assert(out.indexOf('Income and Savings') !== -1, 'the five-column media table is recognised and emitted');
        assert(out.indexOf('Item Type') !== -1, 'the header row is rendered');
        assertTrue(new MediaListConverter().hasMediaTable({ content: [table] }),
            'and it auto-classifies as a Media List (hasMediaTable true)');
    });
