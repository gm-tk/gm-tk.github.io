...
function mrpBuildDoc() {
    var els = {
        ...
        'btn-module-reset': mrpMockEl({ id: 'btn-module-reset', tagName: 'button' }),
        'mode-option-module': mrpMockEl({ id: 'mode-option-module', tagName: 'input' }),
        'mode-option-stitcher': mrpMockEl({ id: 'mode-option-stitcher', tagName: 'input' })
    };
    return {
        els: els,
...
function mrpSpyOutputManager() {
    return {
        ...
        downloadAsZip: function (name) { this.zipCalls.push({ name: name, files: Object.keys(this._store) }); },
...
describe('ModuleResultsPage — Module Development results / download screen', function () {
    ...
    it('shows the writer\'s template download when only the template was converted', function () {
        ...
        assertEqual(items[0].label, 'Writers Template', 'labelled as the Writers Template');
    ...
    it('offers a Download-all (ZIP) control only when 2+ files were produced', function () {
        var one = new ModuleResultsPage();
        one.show([mrpTemplateOut()]);
        assertFalse(one.hasMultipleOutputs(), 'a single output is not "multiple"');
        assertEqual(one.getControlManifest().filter(function (c) { return c.kind === 'download-all'; }).length, 0,
            'no download-all control for a single file');

        var two = new ModuleResultsPage();
        two.show([mrpTemplateOut(), mrpMediaOut()]);
        assertTrue(two.hasMultipleOutputs(), 'two outputs are "multiple"');
        assertEqual(two.getControlManifest().filter(function (c) { return c.kind === 'download-all'; }).length, 1,
            'exactly one download-all control for 2+ files');
    });

    it('renders the Download-all button only with 2+ outputs', function () {
        var ctx1 = mrpBuildDoc();
        new ModuleResultsPage({ document: ctx1.doc }).show([mrpTemplateOut()]);
        assert(ctx1.els['module-results-list'].innerHTML.indexOf('module-download-all') === -1,
            'no ZIP button for a single file');

        var ctx2 = mrpBuildDoc();
        new ModuleResultsPage({ document: ctx2.doc }).show([mrpTemplateOut(), mrpMediaOut()]);
        assert(ctx2.els['module-results-list'].innerHTML.indexOf('module-download-all') !== -1,
            'ZIP button present for 2+ files');
    });

    it('Download-all zips exactly the current outputs (clearing any stale file first)', function () {
        var om = mrpSpyOutputManager();
        om.addFile({ filename: 'STALE_parsed.txt', content: 'from a previous conversion' });
        var page = new ModuleResultsPage({ outputManager: om });
        page.show([mrpTemplateOut(), mrpMediaOut()]);
        var ok = page.downloadAllAsZip();
        assertTrue(ok, 'ZIP download initiated');
        assertEqual(om.zipCalls.length, 1, 'downloadAsZip invoked once');
        assertDeepEqual(om.zipCalls[0].files.slice().sort(),
            ['ENGS301_media_list.txt', 'ENGS301_parsed.txt'].sort(),
            'zip holds exactly the two current outputs — the stale file was cleared out');
        assert(/\.zip$/.test(om.zipCalls[0].name), 'a .zip filename is used');
    });

    it('Download-all is a no-op with a single output', function () {
        var om = mrpSpyOutputManager();
        var page = new ModuleResultsPage({ outputManager: om });
        page.show([mrpTemplateOut()]);
        assertFalse(page.downloadAllAsZip(), 'returns false with fewer than two outputs');
        assertEqual(om.zipCalls.length, 0, 'no zip attempted');
    });
    ...
    it('shows no HTML preview and no phase/template controls on the screen', function () {
        ...
        assertDeepEqual(kinds, ['back', 'download', 'download-all', 'reset'], 'screen owns download + download-all + back + reset controls');
    });

    it('the Back control returns to the front page WITHOUT clearing (non-destructive)', function () {
        var ctx = mrpBuildDoc();
        var page = new ModuleResultsPage({ document: ctx.doc });
        page.show([mrpTemplateOut()]);
        ...
        assertTrue(page.hasOutputs(), 'Back leaves the produced outputs intact');
        assertEqual(page.getOutputs().length, 1, 'outputs preserved so the user can re-check their upload');
    });

    it('the "Convert another module" control fires onReset, clears outputs, and returns to the front page', function () {
        var ctx = mrpBuildDoc();
        var resetCalls = 0;
        var page = new ModuleResultsPage({ document: ctx.doc, onReset: function () { resetCalls++; } });
        page.show([mrpTemplateOut(), mrpMediaOut()]);
        assertTrue(page.hasOutputs(), 'sanity: outputs present before reset');
        ctx.els['btn-module-reset']._fire('click', {});
        assertEqual(resetCalls, 1, 'the onReset hook fired once (the host ModeToggle clears its staged state)');
        assertFalse(page.hasOutputs(), 'produced outputs cleared on a full reset');
        assertFalse(page.isResultsVisible(), 'navigated off the results screen');
        assertTrue(ctx.els['module-results-section'].classList.contains('hidden'), 'results section hidden');
        assertFalse(ctx.els['module-dev-section'].classList.contains('hidden'), 'front page revealed');
    });
