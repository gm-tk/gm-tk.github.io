...
class DocxParser {
    ...
    async parse(file) {
        ...
        // Read optional comments.xml for native Word comments. Additive: when
        // absent (or no CommentExtractor loaded) the comment map is empty and the
        // existing parse output is byte-for-byte unchanged.
        let commentsXml = null;
        const commentsFile = zip.file('word/comments.xml');
        if (commentsFile) {
            commentsXml = await commentsFile.async('string');
        }
        ...
        // Build the native-comment map (additive). Anchors are attached to blocks
        // during the walk; carry-forward runs afterwards.
        this._commentExtractor = (typeof CommentExtractor !== 'undefined') ? new CommentExtractor() : null;
        this._commentMap = (this._commentExtractor && commentsXml)
            ? this._commentExtractor.parseComments(commentsXml)
            : new Map();

        this._progress('Extracting formatted text...');
        this._walkBody(body);

        // Carry a comment off an empty / dropped paragraph to the next kept block
        // (trailing comments → the last block), so nothing is silently lost.
        if (this._commentExtractor && this._commentMap && this._commentMap.size) {
            this._commentExtractor.applyCarryForward(this.content);
        }
    ...
    _handleParagraph(pElement) {
        ...
        const pBlock = { type: 'paragraph', data: para, index: this.paragraphs.length - 1 };
        this._attachParagraphComments(pBlock, pElement);
    ...
    _handleTable(tblElement) {
        ...
        const tBlock = { type: 'table', data: table };
        this._attachTableComments(tBlock, tblElement);
        this.content.push(tBlock);
    }

    // ------------------------------------------------------------------
    // Internal: native Word comment anchor capture (additive — no-ops when no
    // comments / no CommentExtractor, so the existing parse output is unchanged)
    // ------------------------------------------------------------------

    /** Attach comments whose <w:commentRangeStart> sits inside this paragraph. */
    _attachParagraphComments(block, pElement) {
        if (!this._commentExtractor || !this._commentMap || !this._commentMap.size) { return; }
        const ids = this._commentExtractor.anchorIdsInElement(pElement, this.W_NS);
        const arr = this._commentsForIds(ids, null);
        if (arr.length) { block.comments = (block.comments || []).concat(arr); }
    }

    /**
     * Attach comments whose anchor sits inside any row of this table. A media
     * row additionally records the row's hyperlink URL(s) as rowUrl/rowUrls — the
     * key that lets a Media List comment later find its body element.
     */
    _attachTableComments(block, tblElement) {
        if (!this._commentExtractor || !this._commentMap || !this._commentMap.size) { return; }
        const rows = this._getChildrenNS(tblElement, 'tr');
        let all = [];
        for (let i = 0; i < rows.length; i++) {
            const ids = this._commentExtractor.anchorIdsInElement(rows[i], this.W_NS);
            if (!ids.length) { continue; }
            const rowUrls = this._commentExtractor.rowHyperlinkUrls(
                rows[i], this.hyperlinks, this.W_NS, this.R_NS
            );
            all = all.concat(this._commentsForIds(ids, rowUrls));
        }
        if (all.length) { block.comments = (block.comments || []).concat(all); }
    }

    /** Resolve comment ids to {author,text[,rowUrl,rowUrls]} via the parsed map. */
    _commentsForIds(ids, rowUrls) {
        const out = [];
        for (let i = 0; i < ids.length; i++) {
            const c = this._commentMap.get(ids[i]);
            if (!c) { continue; }
            const obj = { id: ids[i], author: c.author, text: c.text };
            if (rowUrls && rowUrls.length) {
                obj.rowUrls = rowUrls.slice();
                obj.rowUrl = rowUrls[0];
            }
            out.push(obj);
        }
        return out;
