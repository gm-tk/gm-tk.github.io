 ...
 * phase/template controls and NO HTML preview, and exposes two return controls:
 * a non-destructive "Back" (toggles section visibility, leaving the staged files
 * intact so the user can re-check them) and a "Convert another module" control
 * that performs a full session reset (clears the staged files + outputs via the
 * host ModeToggle) before returning to the front page.
 *
...
class ModuleResultsPage {
     ...
     * @param {Function} [options.onReset] - Optional hook fired on a full reset
     *        ("Convert another module") so the host ModeToggle can clear its
     *        staged files + front-page state. Omitted in tests (or a spy).
     */
    constructor(options) {
        options = options || {};
    ...
    /** @returns {boolean} Whether ≥2 outputs were produced (so a ZIP is worthwhile). */
    hasMultipleOutputs() {
    ...
    /**
     * The controls the results screen owns, by kind: one `download` per output, a
     * `download-all` (ZIP) control when ≥2 files were produced, plus `back` and
     * `reset`. It declares NO `selector` (phase/template) and NO `preview` control
     * — the screen is purely a download surface. Mirrors the ModeToggle
     * .getControlManifest contract so the invariants are assertable headlessly.
    ...
    getControlManifest() {
        ...
        // A "download all as ZIP" control appears only with ≥2 files (zipping a
        // single file is pointless).
        if (this.hasMultipleOutputs()) { controls.push({ kind: 'download-all' }); }
        controls.push({ kind: 'back' });
        controls.push({ kind: 'reset' });
        return controls;
    }
    ...
    /**
     * Full reset from the results screen ("Convert another module"): discard the
     * produced outputs, fire the onReset hook (the host ModeToggle clears its
     * staged files + re-syncs the front page), then return to the front page.
     * Distinct from returnToFrontPage (the non-destructive "Back", which leaves
     * everything staged so the user can re-check what they uploaded).
     */
    resetSession() {
    ...
    /** Bind the "Convert another module" control to a full session reset. */
    _bindResetControl() {
        var self = this;
        if (this.els.resetBtn && this.els.resetBtn.addEventListener) {
    ...
    _bindModeReset() {
        ...
        [this.els.modeModuleRadio, this.els.modeStitcherRadio].forEach(function (radio) {
    ...
    _renderList() {
        ...
        // When ≥2 files were produced, offer a single "download all as a ZIP"
        // below the per-file buttons (no ZIP control for a lone file).
        if (this.hasMultipleOutputs()) {
            var n = this.outputs.length;
            html +=
                '<div class="download-all">' +
                    '<button id="module-download-all" class="btn btn-primary module-download-all" type="button" ' +
                        'title="Download all ' + n + ' converted files together in one .zip">' +
                        '💾 Download all as a ZIP</button>' +
                '</div>';
        }
        list.innerHTML = html;
        this._bindDownloads(list);
        this._bindDownloadAll(list);
    }
    ...
    /** Bind the "Download all as a ZIP" button (present only with ≥2 outputs). */
    _bindDownloadAll(list) {
        var self = this;
        if (!list.querySelector) { return; }
        var btn = list.querySelector('#module-download-all');
        if (btn && btn.addEventListener) {
            btn.addEventListener('click', function () { self.downloadAllAsZip(); });
        }
    }

    /**
     * Download every produced output together in ONE .zip (only meaningful with
     * ≥2 files). The archive is built from EXACTLY the current outputs — the
     * manager is cleared first so a previous conversion's files can't leak in —
     * and reuses OutputManager.downloadAsZip (JSZip). No-op (returns false) with
     * <2 outputs or when no zip-capable manager is available (e.g. headless).
     * @returns {boolean} Whether the ZIP download was initiated.
     */
    downloadAllAsZip() {
        if (!this.hasMultipleOutputs()) { return false; }
        var om = this._getOutputManager();
        if (!om || typeof om.downloadAsZip !== 'function') { return false; }
        if (typeof om.clear === 'function') { om.clear(); }
        for (var i = 0; i < this.outputs.length; i++) {
            om.addFile({
                filename: this.outputs[i].filename,
                content: this.outputs[i].content,
                type: 'text',
                pageType: 'text'
            });
        }
        om.downloadAsZip(this._zipFilename());
        return true;
    }

    /** ZIP filename — "<MODULE CODE> parsed files.zip" from the outputs' shared
     *  leading code token, else a generic fallback. */
    _zipFilename() {
        var first = (this.outputs[0] && this.outputs[0].filename) || '';
        var code = String(first).split(' ')[0];
        return (/^[A-Za-z0-9]+$/.test(code) ? code : 'PageForge') + ' parsed files.zip';
    }
...
ModuleResultsPage.NEXT_STEPS = [
    ...
    'Into that new chat, upload both files:' +
...
ModuleResultsPage.present = function (toggle, outputs) {
    ...
    if (!toggle._resultsPage) {
        toggle._resultsPage = new ModuleResultsPage({
            document: toggle._document,
            onReset: function () {
                if (typeof toggle.resetSession === 'function') { toggle.resetSession(); }
            }
        });
    }
