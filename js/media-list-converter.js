 ...
 * first row carries the media column labels (Item No., Item Type, Description,
 * Source, URL — plus an optional page-number column whose label varies, e.g.
 * "Pg No." / "WTPg No.", and any optional trailing columns such as ECR approval),
 * matched case-insensitively and whitespace/punctuation-tolerantly, by content
 * not by position. Only that table's data rows are emitted (the column header once at
 ...
 * the standalone toast global (window.pageForgeToast.show) in the browser).
 *
...
class MediaListConverter {
     ...
     *        standalone toast (window.pageForgeToast.show) in the browser.
     */
    constructor(options) {
        options = options || {};
    ...
    convertParsedResult(result) {
        ...
        if (!table) {
            this._notifyError('Media List: no media table found. The document did not contain a recognised media-list header row (Item No., Item Type, Description, Source, URL).');
            return '';
        }
    ...
    /**
     * Whether a parsed result contains a recognised media-list table. Additive
     * helper used to auto-classify an uploaded .docx (Writer's Template vs Media
     * List) when both share one upload container.
     *
     * @param {Object} result - a DocxParser parse result.
     * @returns {boolean}
     */
    hasMediaTable(result) {
        var content = (result && Array.isArray(result.content)) ? result.content : [];
        return !!this._findMediaTable(content);
    }
     ...
     * stable five-column signature). Optional columns — the page-number column
     * (Pg No. / WTPg No.), ECR approval, Activity No., etc. — may be present or
     * absent and never gate the match. Case-insensitive, whitespace/punctuation
     * tolerant, position-independent.
     */
    ...
    _notifyError(message) {
        ...
        // V1 fell back to the Standard App's toast; that class is dropped in this
        // build, so the fallback now targets the standalone toast global (see
        // js/toast.js). The injected `notify` sink still takes precedence.
...
// Media-list column header keys (normalised). REQUIRED is the stable five-column
// signature present in (effectively) every media list. The page-number column
// VARIES in label — "Pg No." is the corpus majority, "WTPg No." / "WT Pg No."
// also occur — and is sometimes ABSENT entirely; other trailing columns (ECR
// approval, Activity No., …) come and go. So none of those gate detection — they
// are OPTIONAL and the extraction is header-driven (it emits whatever columns the
// table actually has). Measured across the finalized corpus: 436/437 media tables
// carry all five REQUIRED labels, while only ~14% carry "WTPg No." (≈86% use
// "Pg No." or omit the page-number column), which is why requiring it was wrong.
MediaListConverter.REQUIRED_HEADERS = ['item no', 'item type', 'description', 'source', 'url'];
MediaListConverter.OPTIONAL_HEADERS = ['pg no', 'wtpg no', 'wt pg no', 'ecr approval', 'activity no', 'lesson activity no'];
