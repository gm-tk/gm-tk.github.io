...
- **Module Development Mode — Session 1 (Mode Shell + Toggle + Front Page)** — Added a top-level two-option mode switch (Module Development default, Standard) via new `js/mode-toggle.js` (388 lines); the Module Development front page exposes two optional `.docx` slots (Writer's Template + Media List) with an Activate button gated on ≥1 file and a stubbed `handleModuleConversion()` for Session 2; Standard `#upload-section` hidden by default and otherwise untouched; 697/697 tests pass (+15 across two new test files). [docs/31-module-development-mode.md](docs/31-module-development-mode.md)
