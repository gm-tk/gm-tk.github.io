...
```
  ...
  - `red_hex_values` — the exact shades the converter treats as "red text" in every case. **Since round 341 a near-red shade is caught automatically** — see `near_red_tag_runs` below — so this list only needs a new entry for a shade OUTSIDE the red band (or when a writer's red must count as red even on runs that carry no bracket).
  - `hyperlinked_tag_runs` (round 342) — `{ "enabled": true, "env": "HYPERTAG_OFF", "head_words": [...], "exclude_words": ["animation"], "link_target_match": "..." }`. Some writers type a media tag as a hyperlink instead of red text — `[audio 1]` in the link blue, linked to the sound file on Drive or SharePoint — so the tag detector never saw it and the bracket shipped as literal text. A hyperlinked run whose bracket opens with one of the listed media words now counts as red, but only when the link points at a media-file carrier (`link_target_match` — the Drive / SharePoint / iStock / YouTube hosts or a media file extension) and the bracket carries none of the `exclude_words`. The head list is the fence: the same writers hyperlink every WORD of a phonics list (`[scissors]`, `[wrist]` …), which is content, not a tag — never widen the list without measuring (`outputs/_measure_r342_hyperlinked_brackets.py` tallies every hyperlinked bracket by its head word and target host). `embed` is deliberately absent (its only hyperlinked runs are the reading-book family the carousel must not half-build).
...
- **Subject_Prefix_Map.json** (round 408, 20 Sept 2026) — the one hand-written input to the rebuild: which subject label a module-code prefix belongs to, for the prefixes the old report folders never filed (the September 2026 intake's CBI / CHI / WJFUN / PWY … and a few older ones such as BLLR and TEDC). The registry builder reads the report folders first and only fills gaps from this file. The label is what the HTML Generator's Reference-module **Subject** filter shows; a prefix with no label anywhere shows under "All subjects" but is never offered as a filter entry. The labels in it are marked PROPOSED until Chris approves them (the decision points are written in its `_meta`).

**When the library grows (an intake), rebuild ALL of these together — never just one.** Round 263 (August 2026) shipped a module with no menus precisely because the index was rebuilt while the Style-Anchor and Menu registries were not; round 408 (September 2026) did the whole set in one pass and the recipe is in `converter-v2/CLAUDE.md` §13 (`build_granular_registry.py`, the per-family Style-Anchor mining, `derive_menu_type.cjs`, `build_convention_registry.py`, the heading lexicon, the feature index, the legacy consensus, then a probe over every module to size the regeneration). Plain-English result of round 408: the picker went from 454 to 552 library modules, every new family got its own measured page shell (which is what turned the 21 WJFUN modules from 2–5 invented pages into the one page their developers built), and the skeleton score rose 53.1 % → 53.7 % with no other gate moving the wrong way.
