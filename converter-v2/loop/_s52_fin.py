#!/usr/bin/env python3
"""_s51_fin.py — session 52 (a copy of _s51_fin.py)'s shared (a copy of _s49_fin.py; the in-flight and no-round anchors widened) FINALISE helper (WSL). One call per shipped engine round:
...
def finalise(*, N, old_build, new_build, entry, config_comment, og9=None, og11=None, og14, gb_note,
    ...
    io.open(A, "a", encoding="utf-8", newline="\n").write(
        f"\n## Position — Before it r{bprev.group(1)} + the prior no-round line (verbatim, s52 r{N})\n\n" + before_it + "\n" + prior_nr + "\n"
        f"\n## Session 52 — r{N} PICK (the in-flight marker, verbatim) + what shipped\n\n" + marker + "\n" + archive_extra + "\n")
