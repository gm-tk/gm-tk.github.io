/** _measure_r352_carmembers.cjs — ROUND 352 PICK measurement (the D10-3 kickoff sequence, carousel): the MEMBER ITEMS of EVERY carousel bundle
 ...
 *      node ../../outputs/_measure_r350_ddcolumn.cjs --merge      → _r352_carmembers.json
...
const SHARD = (k) => path.join(OUT, `_r352_car_shard${k}.json`);
const clean = (s) => String(s ?? "").replace(/\s+/g, " ").trim();
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r352_carmembers.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r352_carmembers.json"), all.length);
	return;
}
...
async function main() {
	...
	InteractiveBuilder.Build = function (args) {
		...
		try {
			seen++;
			if (b?.type === "carousel" || b?.variant === "rotateBanner") {
