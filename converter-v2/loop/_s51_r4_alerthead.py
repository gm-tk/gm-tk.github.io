#!/usr/bin/env python3
"""_s51_r4_alerthead.py (from _s51_r3_acthead.py) — the CALLOUT (alert / important / whakatauki) opener typed in ONE red span with a heading tag, either
...
reference/tests/. Writes outputs/_s51_r4_alerthead.tsv."""
...
ACT = re.compile(r"\[\s*((?:alert|important|whakat\S*|w.nanga|summary box)[^\]]*)\]", re.I)
...
for f in sorted(nspan, key=lambda f: -nspan[f]):
    n = sum(G[f].values()); act = sum(v for k, v in G[f].items() if ("alert" in k))
    print(f"{f:8s} spans {nspan[f]:3d} gold in-alert {act}/{n}  {dict(G[f].most_common(3))} || claude {dict(C[f].most_common(3))}")
with open("../../outputs/_s51_r4_alerthead.tsv", "w") as fh:
