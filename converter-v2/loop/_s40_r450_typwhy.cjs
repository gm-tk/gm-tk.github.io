...
async function main() {
  ...
  const lineSrc = {};
  for (const fn of ["#typing", "#withMembers", "#withMembersRead"]) {
  ...
  for (let i = 0; i < src.length; i++)
    if (src[i].includes("html = this.#typing({ bundle, tpl, renderInline });"))
      src[i] = src[i].replace("html = this.#typing({ bundle, tpl, renderInline });",
        "html = this.#typing({ bundle, tpl, renderInline }); globalThis.__mcqRaw = html ? html.length : 0;");
  const tmp = path.join(__dirname, `_s40_r450_IB_patched_${process.pid}.cjs`);   // one per process — parallel shards raced on a shared copy
  ...
  IB.Build = function (args) {
    ...
    if (b && b.type === "typing") {
      ...
      else if (!args.templates || !args.templates.typing) why = "no multiChoiceQuiz template in the templates handed to Build";
      else if (args.templates.typing.enabled === false) why = "template disabled";
  ...
  for (const [k, v] of last) _l(`TYPWHY\t${k}\t${v.act}\t${v.why}\tcalls=${v.n}\tfirst=${v.first === v.why ? "=" : v.first.slice(0, 60)}`);
}
