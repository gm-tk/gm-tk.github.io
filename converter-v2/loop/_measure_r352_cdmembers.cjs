/** _measure_r352_cdmembers.cjs — ROUND 352 PICK measurement (the D10-3 clickDrop kickoff): the MEMBER ITEMS of EVERY clickDrop bundle
 ...
 *      node ../../outputs/_measure_r350_ddcolumn.cjs --merge      → _r352_cdmembers.json
...
const SHARD = (k) => path.join(OUT, `_r352_cd_shard${k}.json`);
const clean = (s) => String(s ?? "").replace(/\s+/g, " ").trim();
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r352_cdmembers.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r352_cdmembers.json"), all.length);
	return;
}
