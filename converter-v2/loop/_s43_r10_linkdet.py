...
def container(s, pos):
    ...
    return "body"
DET = []; agg = collections.Counter(); mods = collections.defaultdict(set); ex = collections.defaultdict(list); pages = collections.defaultdict(set)
for code in sorted(_corpus.gate_mods(CLAUDE)):
    ...
    for m in LINK.finditer(wt):
        ...
        if fate != "KEPT" and g == "gold <a>": DET.append([code, pg, cont, fate, words, url])
        if len(ex[key]) < (40 if key[1] == "tab-pane" else 3): ex[key].append(f"{code}: «{words[:30]}» {k[:40]}")
import json; json.dump(DET, open("../../outputs/_s43_r10_linkdet.json","w"), ensure_ascii=False, indent=0)
