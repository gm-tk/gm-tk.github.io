#!/usr/bin/env python3
"""ROUND 447 (session 40 Round 1 — D13-5, the journal-button variants) — pre-score the ON pages with the gate's own pairing and match().
For each changed module (_r444_ON_modules.txt): pairs() on the disk (the r446 state), then each pair's SCAFFOLD with the disk page (OLD) and
with the ON page saved by `_r447_probe_run.sh SAVE` (NEW). Prints every mover, the up / down counts, the pp-sum
...
codes = [c.strip() for c in open(os.path.join(O, "_r447_ON_modules.txt")) if c.strip()]
rows = []
for code in codes:
    for n, cp, hp in pairs(code):
        onp = os.path.join(O, "_r447_on", code, os.path.basename(cp))
