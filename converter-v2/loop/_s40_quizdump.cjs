/** _r440_probe.cjs — ROUND 440 in-memory probe (ON = the live registry vs the disk built with the pre-round registry; OFF = --pre _r440_pre, must equal the disk); a verbatim copy of _s33_r426_probe.cjs.
 ...
 *    BAREACT_OFF=1 STUB_OEMBED=1 node --require ./_deflate_raw_polyfill.cjs ../../outputs/_s27_r406_probe.cjs CODES…   (expect: all identical)
 *    STUB_OEMBED=1 node --require ./_deflate_raw_polyfill.cjs ../../outputs/_s27_r406_probe.cjs CODES…                     (the round's diffs)
...
const preIx = process.argv.indexOf("--pre");
const preFile = preIx >= 0 ? process.argv[preIx + 1] : null;   // ROUND 398: the pre-round icon_rule (JSON) — the OFF leg's registry
const codes = process.argv.slice(2).filter((a, k, arr) => !a.startsWith("--") && arr[k - 1] !== "--save" && arr[k - 1] !== "--pre")
...
async function main() {
  ...
  if (preFile) {   // SESSION 28 / TASK 1 OFF leg: --pre <dir> = the pre-rebuild snapshot of the mined registries (outputs/_s28_t3_pre)
    const m = eng.manifest().data_map;   // engine key -> data file name
    let swapped = 0;
    for (const [k, f] of Object.entries(m)) {
      const p = path.join(preFile, path.basename(f));
      if (fs.existsSync(p)) { DataService.Data[k] = JSON.parse(fs.readFileSync(p, "utf8")); swapped++; }
    }
    process.stderr.write(`pre-snapshot: ${swapped} engine data file(s) swapped from ${preFile}\n`);
  }
  const _l = console.log.bind(console); console.log = () => {}; console.warn = () => {}; console.error = () => {};
  eng.loadEngine();
  // SESSION 40 DIAG: every bundle of type $QTYPE (default multiChoiceQuiz), dumped whole as one JSON line (QDUMP …)
  { const IB = globalThis.InteractiveBuilder; const _orig = IB.Build; const seen = new Set();
    IB.Build = function (args) { const r = _orig.call(this, args); const b = args && args.bundle;
      if (b && b.type === (process.env.QTYPE || "multiChoiceQuiz")) {
        const key = `${args.run && args.run.moduleCode}#${b.index}`;
        if (!seen.has(key)) { seen.add(key);
          const mem = (x) => ({ type: x.type, cls: x.parse?.class ?? null, tag: x.parse?.primary?.tag ?? null, dir: x.parse?.primary?.directive ?? null,
            text: String(x.text ?? ""), after: String(x.blackAfter ?? ""), marks: x.block?.marks ?? null,
            rows: x.type === "table" ? (x.block?.rows ?? null) : null, cellMarks: x.type === "table" ? (x.block?.cellMarks ?? null) : null });
          _l("QDUMP " + JSON.stringify({ code: args.run && args.run.moduleCode, idx: b.index, built: !!r, act: b.activityId ?? null,
            extra: b.extraTypes ?? [], instr: b.instructions ?? [], openers: (b.openerItems || []).map(mem), members: (b.memberItems || []).map(mem) }));
        }
      }
      return r; };
  }
