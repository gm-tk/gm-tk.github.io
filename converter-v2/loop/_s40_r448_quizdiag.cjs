...
async function main() {
  ...
  eng.loadEngine();
  // SESSION 40 R448 DIAG: every quiz-type bundle's members — type, class, text, blackAfter, marks
  { const IB = globalThis.InteractiveBuilder; const _orig = IB.Build; const cut = (s, n = 110) => String(s ?? "").replace(/\s+/g, " ").trim().slice(0, n);
    const QUIZ = new Set(["multiChoiceQuiz", "typing", "dropDown", "dropQuiz", "radioQuiz", "reorder", "selectionBox"]);
    IB.Build = function (args) { const r = _orig.call(this, args); const b = args && args.bundle;
      if (b && QUIZ.has(b.type)) {
        _l(`BUNDLE ${args.run && args.run.moduleCode} #${b.index} ${b.type} built=${!!r} act=${b.activityId ?? "-"}`);
        for (const m of [...(b.openerItems || []), ...(b.memberItems || [])]) {
          const p = m.parse?.primary; const mk = m.block?.marks ? JSON.stringify(m.block.marks).slice(0, 120) : "";
          const cmk = m.block?.cellMarks ? "cellMarks:" + JSON.stringify(m.block.cellMarks).slice(0, 160) : "";
          _l(`   ${m.type}\tcls=${m.parse?.class ?? "-"}\ttag=${p?.tag ?? "-"}\tdir=${p?.directive ?? "-"}\ttext=${cut(m.text)}\tafter=${cut(m.blackAfter)}\t${mk}${cmk}`);
        }
      }
      return r; };
  }
