...
async function main() {
  ...
  InteractiveBuilder.Build = function (args) {
    ...
    if (b?.type === "carousel") {
      ...
      (b.memberItems || []).forEach((m, i) => _l(`   [${i}] ${m.type} tag=${m.parse?.primary?.tag ?? m.tag ?? "-"} dir=${m.parse?.primary?.directive ?? "-"} cls=${m.parse?.class ?? "-"} text=${String(m.text ?? "").replace(RED, "").trim().slice(0, 40)} after=${String(m.blackAfter ?? "").replace(RED, "").trim().slice(0, 40)}`));
      if (out != null) _l("   OUT:", String(out).replace(/\s+/g, " ").slice(0, 300));
    }
    return out;
  };
