#!/usr/bin/env python3
"""ROUND 456 (session 41 Round 4 — the mid-page lesson heading opens its page) — pre-score the ON corpus with the gate's own
...
the ON pages saved by `_r456_probe_run.sh SAVE` (outputs/_r456_on/<CODE>). Prints every pair on both sides, the module sums,
and the corpus-mean estimate on the NEW population (pairs 2477 + the added pairs).
Run from CONVERTER_V2/reference/tests under WSL: python3 ../../outputs/_r456_prescore.py
"""
import os, sys
...
codes = [c.strip() for c in open(os.path.join(O, "_r456_ON_modules.txt")) if c.strip()]
_mdir = _corpus.mdir
ON = {"on": False}
def mdir(root, *parts):
    if ON["on"] and os.path.abspath(root) == os.path.abspath(CLAUDE) and parts and parts[0] in codes:
        return os.path.join(O, "_r456_on", parts[0])
...
BASE_MEAN, BASE_PAIRS = 54.7680, 2487
