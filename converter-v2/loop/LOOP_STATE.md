...
**Next session starts with:** (1) Chris's decisions — unchanged and NOT re-asked: (a) the EQUATION FORM (329 Word equations dropped; MathML per the gold + the V1.5 finding vs LaTeX per KB 05A), (b) stickyNav, (c) decision 5 interactives (the corpus's largest class by far), (d) decision 4 title-pair order, (e) decision 1 c47, (f) the KB table form 05D vs 06 §6, (g) the CED revision-brief modules, (h) NEW — the `alertPadding` activity class (KB 01F table vs gold 0.81 plain; recommendation: leave plain until the KB makes it a rule). (2) Without a decision, the ONE measured candidate above the floor is **c75's untagged hyperlink buttons** (452 gold `externalButton`s / 255 pages whose label phrase is a plain hyperlink in the WT): write a LIVE-extractor probe (`DocxExtractor.Extract` → `block.links`, never the `_parsed.txt` dumps) that classifies every standalone hyperlink line — its own paragraph, phrase + target, no other text — and measures the gold's form for it (externalButton vs inline `<a>` vs plain); ship only if ≥ 0.60 in a template/subject group. (3) After that the derivable ≥ 20-page queue is exhausted for gate-moving work; the remaining measured candidates are under the floor (the box-less `[MTKquiz]` shells ~17 pages, the c79 module-title fallbacks ~24 across four source tags, the 172 empty hand-off boxes, the nested-quiz-in-accordion class CEDW501 surfaced, the crumb-less Inquiry dialects). The full-ship backstop is due after 5 more scoped ships (ledger: 3 since r334).
