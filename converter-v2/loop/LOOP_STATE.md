...
**Next session starts with:** the standing §7 kickoff (`/loop-start`); health check (`verify_after_transfer.sh` PASS — engine + gates manifests refreshed at r355; `bash CONVERTER_V2/outputs/_round_close.sh rNNN` is the one-step mirror + manifests + verify for every round-close); `git status` in pageforge-site: expect a CLEAN tree at the session-15 stop commit (r355 = 7d6757a beneath it) — nothing to reconcile; note the KB HEAD (§0) against `KB_AMALGAMATION_STATUS.md` (469f496 at this stop); every engine tool runs under WSL (`wsl.exe -e bash -lc 'cd /mnt/c/Users/Gavin/TeKura/FINAL_MODULE_DATA/CONVERTER_V2/reference/tests && …'`), engine / data / JSON edits go through a scratchpad script written with the editor (never a Git-Bash heredoc), repo files are read with `sed -n`. **The loop stopped on §4 EXHAUSTION** — the derivable queue is empty at the 20-page floor; a new session needs one of: **(a) a NEW instrument or class named by Chris** (the standing candidates, each under the floor on its own: the per-builder consumption seam `bundle._consumedMembers` that would make the r353 members rule exact — flipCard 16 + modal 10 + accordion 9 + carousel in-table 12 + dropDown 12 sites regain their paragraphs, 2–3 engine rounds; the flipCard multi-pair table that builds only its first Front / Back pair — 7 sites, 4 shipped builds losing cards; the widget seam rendering a trailing `[button]` / `[audio]` through the media / button emitters — 6 + 10 sites; 03B's `col-md-12 col-12` wrapper for column layouts — a KB decision, gold 0.20); **(b) a D10-3 kickoff below the floor** if Chris lowers it (the largest un-built shapes: clickDrop's horizontal heading / link / words table 17, the image-button rows 13, the dragAndDrop word-tile sentences 37 with the answer present in ≈ half, the image column-sort 13); or **(c) the KB moving** (a new CL row ≥ 20 pages). Until one of those, `/loop-start` will re-measure and stop again on exhaustion. The scorecard baseline for every later round is the r355 state (`_r355_sk_final.json`: SCAFFOLD 51.979 % = 56.6 % of achievable — the corrected instrument; RAW 36.711 %; 1955 pairs; ceiling 91.9 %); the ship ledger is at 0 (FULL at r354); the plateau window 0 of 3. Every recorded decision (D10-1 … D10-9) is settled — never re-ask. Queued tooling (no round needed unless Chris asks): narrow `_coverage_dashboard.py`'s freshness guard to the engine's `data_map` (CLAUDE.md §16 — it still prints three false WARNs after a `Config.js` bump); the free-body Word comment anchored to a non-first list item (AGH1005 2.0, r352).
