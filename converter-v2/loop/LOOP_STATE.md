...
- Standing facts: AppVersion 260619.37; every toggle is listed in CLAUDE.md §11; the engine + gate checksum manifests (`_MIGRATION/CHECKSUMS__engine.txt` / `CHECKSUMS__gates.txt`) are refreshed at every ship (a `.pre-rNNN.bak` copy kept).
...
**Next session starts with:** the standing `/loop-start`; health check (locks, `verify_after_transfer.sh` PASS — manifests refreshed at r366, `wc -c`); `git status` in pageforge-site: expect a CLEAN tree at the session-21 commits (r364 6114fbb, r365 990facf, r366 on top); `DIFF_QUEUE.md` is current (re-mined 02:27 on the r366 corpus); the chrome rows are all dispositioned (the archived r365 PICK + Declined classes); PICK from the r366 residues above, re-running `_measure_r366_actmiss.py` (WT unpack fixed) for the class-C / lost split of the 'not on Claude's page' activity lines.
