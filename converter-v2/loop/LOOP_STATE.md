...
- Every shipped round r314–r365 is ONE line in the Round log below (r314–r356 in `LOOP_STATE_ARCHIVE.md` 'Round log (archived)'); every per-round PICK + what-shipped section is in the archive (`grep -n '^## ' LOOP_STATE_ARCHIVE.md`).
- LAST SHIPPED: **r365** (build 260619.36, 18 Sept ≈01:55, session 21 Round 2) — SCOPED regeneration (12 modules), ledger scoped ship #1 since the r364 full. **Corpus = r365.** Gates at r365 (`gate_baseline.json`): skeleton **52.651 %** / ≥50 **1121** / ≥75 **173** / ≥90 **15** @ 1956 pairs; RAW 37.166 %; compare_structure exact 11631 (EXTRA 175 / MISSING 617 / row-wrap 23); body_compare 197 / 42 / 4 / 152 (named at r364); every widget verifier at its recorded baseline. Ceiling 91.9 % → 57.3 % of achievable. `DIFF_QUEUE.md` = the r364 corpus (01:17; re-mine after the next FULL regeneration).
- Plateau window (§4): **1 of 3** (r364 +0.290pp; r365 +0.014pp with ≥50 +3 — counted conservatively).
- IN PROGRESS: none (r365 shipped). Every CHROME row of the r364 queue is dispositioned (see the archived r365 PICK); the next PICK decomposes the activity / body rows (#534 MISSING `p`, #539 MISSING `h3`, #535 EXTRA `p`) by mechanism before choosing.
...
- Standing facts: AppVersion 260619.36; every toggle is listed in CLAUDE.md §11; the engine + gate checksum manifests (`_MIGRATION/CHECKSUMS__engine.txt` / `CHECKSUMS__gates.txt`) are refreshed at every ship (a `.pre-rNNN.bak` copy kept).
...
**Next session starts with:** the standing `/loop-start`; health check (locks, `verify_after_transfer.sh` PASS — manifests refreshed at r365, `wc -c`); `git status` in pageforge-site: expect a CLEAN tree at the session-21 commits (r364 6114fbb, r365 on top); the miner is due (the last re-mine was on the r364 corpus; r365 was scoped) — re-run it, then PICK: the chrome rows are all dispositioned (the archived r365 PICK + Declined classes); decompose the activity / body rows (#534 MISSING `p`, #539 MISSING `h3`, #535 EXTRA `p`) by mechanism before choosing.
