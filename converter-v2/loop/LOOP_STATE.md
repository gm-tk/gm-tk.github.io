...
- Standing facts: AppVersion 260619.38; every toggle is listed in CLAUDE.md §11; the engine + gate checksum manifests (`_MIGRATION/CHECKSUMS__engine.txt` / `CHECKSUMS__gates.txt`) are refreshed at every ship (a `.pre-rNNN.bak` copy kept).
...
**Next session starts with:** the standing `/loop-start`; health check (locks, `verify_after_transfer.sh` PASS — manifests refreshed at r367, `wc -c`); `git status` in pageforge-site: expect a CLEAN tree at the session-21 commits (r364 6114fbb, r365 990facf, r366 53f96a6, r367 on top); `DIFF_QUEUE.md` is current (re-mined 02:27 on the r366 corpus; r367 changed nothing); PICK: the captured LIST leads (`_measure_r367_leadlist.py` → `_r367_leadlist.{json,log}`), then the r366 residues; the chrome rows are all dispositioned (the archived r365 PICK + Declined classes).
