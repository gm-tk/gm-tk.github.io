...
- Every shipped round r314–r364 is ONE line in the Round log below (r314–r356 in `LOOP_STATE_ARCHIVE.md` 'Round log (archived)'); every per-round PICK + what-shipped section is in the archive (`grep -n '^## ' LOOP_STATE_ARCHIVE.md`).
- LAST SHIPPED: **r364** (build 260619.35, 18 Sept ≈01:20, session 21 Round 1 — committed at the end of the round) — FULL regeneration, ledger FULL (counter 0). **Corpus = r364.** Gates at r364 (`gate_baseline.json`): skeleton **52.637 %** / ≥50 **1118** / ≥75 **173** / ≥90 **15** @ 1956 pairs; RAW 37.162 %; compare_structure exact 11631 (= the matched pool −14 at r364; EXTRA 175 / MISSING 617 / row-wrap 23); body_compare 197 / 42 / 4 / 152 (named); every widget verifier at its recorded baseline. Ceiling 91.9 % → 57.3 % of achievable.
- Plateau window (§4): **0 of 3** (r363 +0.081pp, r364 +0.290pp).
- IN PROGRESS: none (r364 shipped). The next PICK reads `DIFF_QUEUE.md` (re-mined 2026-09-18 01:17 on the r364 corpus: 167 candidates) and skips every row already dispositioned in the archived PICK sections (sessions 19–21) and the Declined / Blocked lists below.
...
- Standing facts: AppVersion 260619.35; every toggle is listed in CLAUDE.md §11; the engine + gate checksum manifests (`_MIGRATION/CHECKSUMS__engine.txt` / `CHECKSUMS__gates.txt`) are refreshed at every ship (a `.pre-rNNN.bak` copy kept).
...
**Next session starts with:** the standing `/loop-start`; health check (locks, `verify_after_transfer.sh` PASS — manifests refreshed at r364, `wc -c`); `git status` in pageforge-site: expect a CLEAN tree at the session-21 commit(s) on top of r364; then the PICK from `DIFF_QUEUE.md` (re-mined 01:17 18 Sept on the r364 corpus, 167 candidates), chrome first, skipping the rows the archived PICK sections (sessions 19–21) and the Declined / Blocked lists already disposition.
