#!/usr/bin/env python3
"""_s49_fin.py — session 49's shared FINALISE helper (WSL). One call per shipped engine round:
...
def finalise(*, N, old_build, new_build, entry, config_comment, og9=None, og11=None, og14, gb_note,
    ...
    others = [i for i, l in enumerate(L) if l.startswith("- **No round in flight**")]
    ...
    io.open(A, "a", encoding="utf-8", newline="\n").write(
        f"\n## Position — Before it r{bprev.group(1)} + the prior no-round line (verbatim, s49 r{N})\n\n" + before_it + "\n" + prior_nr + "\n"
        f"\n## Session 49 — r{N} PICK (the in-flight marker, verbatim) + what shipped\n\n" + marker + "\n" + archive_extra + "\n")
