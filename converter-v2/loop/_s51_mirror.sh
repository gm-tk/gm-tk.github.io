#!/usr/bin/env bash
# SESSION 51 — mirror (a copy of _s46_mirror.sh) the loop artefacts + one round's tools / logs into pageforge-site/converter-v2/loop/ and prove each copy
# byte-identical (cmp); append the round's README row once. Usage (Git Bash or WSL): bash _s51_mirror.sh <TAG> <ROWFILE> [EXTRA outputs/ files…]
