...
for(const t of ["[Wananga/Talanoa box]","[Wānanga/Talanoa box]","[Banner - Wānanga/Talanoa box]","[Wananga box]","[wānanga box]","[Talanoa/Wananga box]","[Wanana/Talanoa box]","[Wananaga/Talanoa box]","[Wananga / Talanoa box]","[Wananga/ Talanoa box]","[Rolling Banner - Wānanga/Talanoa box]","[wananga]","[talanoa]","[cultural alert]","[whakatauki]"]){
  let r=null;try{r=norm.Parse(t);}catch(e){console.log(t,"ERR",e.message);continue;}
  const p=r&&r.primary;
  const wt = p? (norm.GetWidgetTypes(p.tag)||[]):[];
  console.log(t.padEnd(14), JSON.stringify(r.tags.map(x=>[x.tag,x.how,x.remainder])), "instr=",r.instructionFragment, "class=",r.class);
}
