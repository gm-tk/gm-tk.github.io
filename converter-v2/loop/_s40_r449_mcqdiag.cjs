...
async function main() {
  ...
  eng.loadEngine();
  // SESSION 40 R449 DIAG: every multiChoiceQuiz bundle, dumped whole as one JSON line (MCQDUMP …)
    ...
    IB.Build = function (args) { const r = _orig.call(this, args); const b = args && args.bundle;
      if (b && b.type === "multiChoiceQuiz") {
        ...
        if (!seen.has(key)) { seen.add(key);
          ...
          _l("MCQDUMP " + JSON.stringify({ code: args.run && args.run.moduleCode, idx: b.index, built: !!r, act: b.activityId ?? null,
