#!/usr/bin/env python3
"""ROUND 453 (session 41 Round 1 — the table-cell title bar opens the document) — pre-score the ON corpus with the gate's own
...
the ON pages saved by `_r453_probe_run.sh SAVE` (outputs/_r453_on/<CODE>). Prints every pair on both sides, the module sums,
and the corpus-mean estimate on the NEW population (pairs 2477 + the added pairs).
Run from CONVERTER_V2/reference/tests under WSL: python3 ../../outputs/_r453_prescore.py
"""
import os, sys
...
codes = [c.strip() for c in open(os.path.join(O, "_r453_ON_modules.txt")) if c.strip()]
_mdir = _corpus.mdir
ON = {"on": False}
def mdir(root, *parts):
    if ON["on"] and os.path.abspath(root) == os.path.abspath(CLAUDE) and parts and parts[0] in codes:
        return os.path.join(O, "_r453_on", parts[0])
