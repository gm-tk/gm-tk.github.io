...
async function main() {
  ...
  const lineSrc = {};
  for (const fn of ["#multiChoiceQuiz", "#withMembers", "#withMembersRead"]) {
  ...
  for (let i = 0; i < src.length; i++)
    if (src[i].includes("html = this.#multiChoiceQuiz({ bundle, tpl, renderInline });"))
      src[i] = src[i].replace("html = this.#multiChoiceQuiz({ bundle, tpl, renderInline });",
        "html = this.#multiChoiceQuiz({ bundle, tpl, renderInline }); globalThis.__mcqRaw = html ? html.length : 0;");
  const tmp = path.join(__dirname, `_s40_r449_IB_patched_${process.pid}.cjs`);   // one per process — parallel shards raced on a shared copy
  ...
  IB.Build = function (args) {
    ...
    if (b && b.type === "multiChoiceQuiz") {
      ...
      else if (!args.templates || !args.templates.multiChoiceQuiz) why = "no multiChoiceQuiz template in the templates handed to Build";
      else if (args.templates.multiChoiceQuiz.enabled === false) why = "template disabled";
  ...
  for (const [k, v] of last) _l(`MCQWHY\t${k}\t${v.act}\t${v.why}\tcalls=${v.n}\tfirst=${v.first === v.why ? "=" : v.first.slice(0, 60)}`);
}
