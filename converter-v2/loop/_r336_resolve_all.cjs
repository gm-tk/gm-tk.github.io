// ROUND 336 — dump the resolved module_code pattern for every module with a Claude dir (before/after the registry edit)
...
for (const tmpl of fs.readdirSync(CL)) {
  ...
  for (const code of fs.readdirSync(d)) {
    ...
    try { const r = ModuleResolver.Resolve(code, run); out[code] = { tmpl, module_code: r.module_code, h1_count: r.h1_count }; }
