#!/usr/bin/env python3
"""Session 27 Round 6 — CLAUDE'S EXTRA ACTIVITY BOXES (287 lesson pages where Claude ships more top-level boxes than the gold) — the mirror of _s27_r6_goldboxes.py.
For every Claude top-level activity box whose first heading text is NOT the first heading of any gold box on the paired page:
...
subject; the writer's opener for the heading where the parsed WT has it.  wsl: python3 _s27_r6_claudeboxes.py"""
import os, sys, re
...
for code in sorted(fam):
    ...
    for n, cp, hp in pairs(code):
        ...
        if len(cb) <= len(gb): continue
        cheads = {first_heading(b)[1] for b in gb}
        # the gold's headings anywhere + their context
        cctx = {}
        def cw(x):
...
print(f"claude extra boxes on claude-more lesson pages: {TOT}")
print("the gold's rendering of the box's heading:", dict(CL.most_common()))
