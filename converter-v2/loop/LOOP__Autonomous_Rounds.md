...
7. **`SESSION_28__Pre_Loop_Summary_2026-09-20.md`** (folder root) — the pre-loop session that
   ...
   **A DIRTY TREE OUTRANKS A CLEAN-LOOKING POSITION LINE (added 22 September 2026, after the
   ...
   state file's Position section names no round in flight, the previous session died mid-round and the
...
is pre-intake. Current (each cell carries the round that last changed it — r504, 25 September 2026; the history since the 22 Sept Round 0d: the 12 XOTP modules and the 38 pre-intake never-converted modules are IN; r426 folded 45 over-split pages into 8 single pages; r427 changed chip text only; r440 (D13-6) built MXFUN01 / BLL240 / CEDT207 as one page each and re-paired the five dual-build golds — 2,699 → 2,666 pages, 2,491 → 2,470 pairs; r442 paired six more lesson pages → 2,476):

| | before the intake | now |
|---|---:|---:|
| Claude module dirs | 416 | **545** |
| Claude pages | 2,109 | **2,673** (r504, 25 Sept: −6 — CEDT301 built as ONE tabbed page, D15-17; r470, 24 Sept: +4 — TRR115 converts, its table-cell `[TITLE BAR]` now recognises the Writers Template; r459, 24 Sept: −16 — the module introduction stays on the overview, 17 modules; r458: −9 — a placement note no longer splits BLLR201–203 / ART1006 / HIS1001; r457: −17 — the section-label marker no longer splits HIS1002 / PES1008; r456: +41 — the mid-page lesson heading opens its page, 22 modules; r453: +10 — the TRR table-cell title bar) |
| gold dirs | 454 | **552** |
| Writers Template / Media List docx | 619 | **762** |
| skeleton paired pages | 1,956 | **2,486** (r504, 25 Sept: −5 — CEDT301's six split pairs → its one page, D15-17, a NAMED population change; r470, 24 Sept: +4 — TRR115's four pages; r465, 24 Sept: −37 — CHI1003 / 1004 / 1005 + JPN1004 leave the SCORED population by D14-21, `compare_exclusions.txt`; the dirs and pages stay in the census above; r459: +6 net; r458: −1 — ART1006_4.0; r457: −1 — PES1008's lesson 3 is one page, the human's 3.0 / 3.1 split; r456, 24 Sept: +33 net — 37 new lesson pages, 4 lost to content pairing; r453: +10 — the restored TRR pages; r447: +1 — PWY1002_2_2_0 pairs by content once its journal heading is the gold's own) |
...
**1,956 → 2,349** (2,353 on 21 Sept, after r423), compare_structure matched **13,794 → 15,668**, body/defect pages
...
(placed / no source / needs Chris / rejected). **As of 22 Sept 2026 every leftover in both areas is
...
## 1g. THE PLACEMENT CENSUS — where the human puts it vs where Claude puts it (Chris, 24 September 2026 — D14-S1, STANDING)

**What Chris said** (after session 41, quoting the loop's own r460 finding — "the gold standard moves
Knowledge/Practices sections out of the Overview pane in 31 of 36 modules (21 into their own tabs, 10 into
Information), whereas my output kept all 36 in Overview"): *"This is EXACTLY the sort of thing I want Claude to be
extra vigilant in identifying and fixing as much as possible, so continue checking for these types of
discrepancies wherever possible in order to then have future loops configured to patching up the PageForge to make
sure this is correctly converting EVERY aspect of the module."*

**Why the ordinary instruments miss it.** The skeleton SCAFFOLD collapses every widget-class container to ONE
`WIDGET` line — and the module menu's `div.tabs` is one of them, so a whole section sitting in the wrong menu pane,
or missing from it, scores +0.0000pp (r460: 28 pages changed, SCAFFOLD exact). The miner's module-menu census counts
TEXT PRESENCE, not position. compare_structure reads `#body` only. So a placement defect of any size can hide in the
menu, in tabs / accordions / carousels, and in hand-off boxes that swallow the text around them.

**The rule (every session, every PICK pass):**
1. **A placement lane.** Beside the miner's rows (§3 step 1), take one pass that locates CONTENT, not tags: a named
   section (by its heading) or a paragraph (by its text), found on the gold page and on Claude's page, compared by
   its CONTAINER PATH — header menu › tab-pane N / body › row › col / activity box / widget raw / absent. Report,
   per region and per template / subject family, the share the gold puts in each container against Claude's. A
   consistent gold placement (≥ 0.60 in a family or template group, or a KB rule) at the floor (chrome 10 modules,
   body 20 pages) is a CANDIDATE even when the skeleton cannot see it; it is judged on that region's own compare (the
   worked example is `outputs/_r460_rawmenu.py`, the menu-only skeleton, gold vs Claude) with every protected gate
   held — a PICK that is skeleton-blind by design says so and neither counts toward nor resets the plateau window.
2. **Tools to start from:** `outputs/_r460_rawmenu.py` (menu-only skeleton), `_s41_r8_kppane.py` (which pane holds a
...
3. **Record everything it finds**, below the floor included, as a line in `LOOP_STATE.md`'s "Follow-up candidates"
   ...
   "Learning Intentions" panes, each module a different authoring form) is the standing example.
4. **It is the SEVENTH lane of §4's exhaustion test.** An exhaustion verdict names what the placement census found.
5. The scored population is `_corpus.gate_mods()` — modules Chris has ruled no-source (D14-21: CHI1003 / CHI1004 /
   CHI1005 / JPN1004, and the list in `compare_exclusions.txt`) are never measured, targeted or developed.
...
- **Regeneration.** The message that started this session carries the code `REGENERATE CORPUS`
  ...
  differ`; a parked patch rides in an ordinary round (§3 step 1), a full-corpus regeneration driven by
  a corpus-wide CHANGE included — only the change-free backstop may not carry one.**
...
1. **PICK.** Read **`DIFF_QUEUE.md` first** (§1d — re-run the miner if it is older than the
   ...
   then the latest intake's §7 round order and `LOOP_STATE.md`'s "Follow-up candidates" — and,
   standing since 24 Sept 2026 (D14-S1), one PLACEMENT CENSUS pass (§1g): where the gold puts each
...
- **Exhaustion.** ALL of: `DIFF_QUEUE.md` exists, was produced by the §1d miner on the CURRENT
  ...
  **Any exhaustion verdict reached before the latest Round 0d (§1f) is VOID** — the date in the newest
  ...
  it 168 → 188 candidate rows (183 after round 408), and the 22 Sept Round 0d added 38 more modules. Exhaustion may only be declared on a miner
