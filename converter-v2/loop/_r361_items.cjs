...
const RX = /nav\b|navigation|\bsection|think like|\bbe a\b|introduction|\bintro\b|end tab|landing|fundamental content|end page|\[h[1-3]\]/i;
  ...
  for (const code of codes) {
    ...
    run.pages.forEach((page, pi) => {
      ...
      items.forEach((it, i) => {
        ...
        _l(`  [${String(i).padStart(4)}] ${String(it.type).padEnd(10)} tag=${clip(p?.tag, 14).padEnd(14)} dir=${clip(p?.directive, 16).padEnd(16)} cb=${it.consumedBy ?? "-"} f=${it._inqSection ?? "-"}/${it._inqSectionConsume ? "c" : "-"} | ${clip(it.text, 70)} ‖ ${clip(it.blackAfter, 40)}`);
      });
    });
  }
}
