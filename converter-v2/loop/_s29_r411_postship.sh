#!/usr/bin/env bash
# SESSION 29 ROUND 2 (engine r411, the tile menu ROW+COL shell) — post-regeneration housekeeping (the r408 sequence minus what
# scoped_ship.sh --commit already did: the fast-loop baseline patch, the content-manifest refresh and the ledger record-scoped).
# reference/tests under WSL: bash ../../outputs/_s29_r411_postship.sh
...
} > $O/_s29_r411_selftests.log 2>&1; echo "  rc=$?  PASS/GREEN lines: $(grep -c 'PASS\|GREEN' $O/_s29_r411_selftests.log)  FAIL lines: $(grep -c 'FAIL' $O/_s29_r411_selftests.log)"
echo "[$(date +%T)] feature index"
{ node --require ./_deflate_raw_polyfill.cjs build_feature_index.cjs --rehtml; node --require ./_deflate_raw_polyfill.cjs build_feature_index.cjs --merge; node --require ./_deflate_raw_polyfill.cjs build_feature_index.cjs --selftest; } > $O/_s29_r411_index.log 2>&1; echo "  rc=$?"; tail -3 $O/_s29_r411_index.log
echo "[$(date +%T)] ship ledger check"
python3 _ship_ledger.py check 2>&1 | tail -2
echo "[$(date +%T)] DIFF MINER (§1d — after every regeneration)"
cp ../../../DIFF_QUEUE.md $O/_diff_queue_pre_r411.md
