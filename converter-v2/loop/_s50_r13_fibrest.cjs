...
async function main() {
	...
	InteractiveBuilder.Build = function (args) {
		...
		if (b?.type === "dragAndDrop" && !out) {
			...
			if (!fib.length) return out;
			const sig = mem.map((m) => m?.type === "black" ? "B" : m?.type === "tag" ? (m.parse?.class === "noise" ? "a" : m.parse?.class === "instruction" ? "I" : "T:" + (m.parse?.primary?.tag ?? m.parse?.class ?? "?")) : (m?.type ?? "?")).join(" ").slice(0, 140);
			_l(`${code}\t#${b.index}\tx${b.extraTypes?.length ?? 0}\t${sig}\tfibLines ${fib.length}\tredWords ${fib.reduce((n, L) => n + L.red.length, 0)}\ttable ${hasTable ? 1 : 0}\tmedia ${media ? 1 : 0}\t${opener}`);
		}
		return out;
	};
