import json, sys
a=json.load(open('_r346_sk_final.json'))['per_page']; b=json.load(open('_r347_sk_final.json'))['per_page']
...
def raw(v):
    ...
    return None
aff=set(l.strip() for l in open('_affected_r347_modules.txt') if l.strip())
mv=[]
for k in A:
    if k in B:
        x,y=sc(A[k]),sc(B[k])
