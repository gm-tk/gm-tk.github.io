#!/usr/bin/env python3
"""ROUND 444 (session 39 Round 6 — D13-7, the activity numbers) — pre-score the ON pages with the gate's own pairing and match().
...
with the ON page saved by `_r444_probe_run.sh SAVE` (NEW). Prints every mover, the up / down counts, the pp-sum
...
codes = [c.strip() for c in open(os.path.join(O, "_r444_ON_modules.txt")) if c.strip()]
rows = []
for code in codes:
    for n, cp, hp in pairs(code):
        onp = os.path.join(O, "_r444v2_on", code, os.path.basename(cp))
