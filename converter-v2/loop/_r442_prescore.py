#!/usr/bin/env python3
"""ROUND 442 (session 39 Round 4 — D13-8) — pre-score the ON pages with the gate's own pairing and match().
For each of the twelve: pairs() on the disk (the r441 state), then each pair's SCAFFOLD with the disk page (OLD) and
with the ON page saved by `_r442_probe_run.sh SAVE` (NEW). Prints every mover, the up / down counts, the pp-sum
and the corpus-mean estimate (pp-sum / 2470 pairs).
...
codes = [c.strip() for c in open(os.path.join(O, "_affected_r442.txt")) if c.strip()]
rows = []
for code in codes:
    for n, cp, hp in pairs(code):
        onp = os.path.join(O, "_r442_on", code, os.path.basename(cp))
...
print(f"movers {len(rows)}: {up} up / {dn} down, pp-sum {pp:+.1f}  ->  corpus mean ≈ {pp/2470:+.4f}pp over 2470 pairs")
