...
wc -l < $O/_s43_wl_modules.txt
rm -f $O/_s43_wl_shard*.json
for k in 0 1 2 3 4 5 6 7; do
  STUB_OEMBED=1 node --require ./_deflate_raw_polyfill.cjs $O/_s43_widgetloss.cjs --shard $k 8 $(cat $O/_s43_wl_modules.txt) > $O/_s43_wl_shard$k.log 2>&1 &
done; wait
node $O/_s43_widgetloss.cjs --merge
if [ -n "$TAG" ]; then mv $O/_s43_widgetloss.json $O/_s43_widgetloss_$TAG.json; fi
tail -q -n 1 $O/_s43_wl_shard*.log
echo "[$(date +%T)] WL_DONE"
