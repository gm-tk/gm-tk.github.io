...
const RX = /spot the place value|item # 7|drop down selection|comparing 1000s|What is the value of the five/i;
  ...
  for (const code of codes) {
    ...
    run.pages.forEach((page, pi) => {
      ...
      items.forEach((it, i) => {
        ...
        _l(`  [${String(i).padStart(4)}] ${String(it.type).padEnd(10)} tag=${clip(p?.tag, 14).padEnd(14)} nums=${JSON.stringify(it.parse?.numbers ?? [])} dir=${clip(p?.directive, 16).padEnd(16)} cb=${it.consumedBy ?? "-"} links=${JSON.stringify(it.block?.links ?? []).slice(0, 110)} f=${it._inqSection ?? "-"}/${it._inqSectionConsume ? "c" : "-"} | ${clip(it.text, 70)} ‖ ${clip(it.blackAfter, 40)}`);
      });
    });
  }
}
