/** _r448_probe.cjs (= _r440_probe.cjs + the {CODE}_interactives.txt worklist compared too) — was _r440_probe.cjs — ROUND 440 in-memory probe (ON = the live registry vs the disk built with the pre-round registry; OFF = --pre _r440_pre, must equal the disk); a verbatim copy of _s33_r426_probe.cjs.
...
async function main() {
  ...
  for (const code of codes) {
    ...
    for (const o of run.outputs) {
      if (!/.(html|txt)$/.test(o.filename)) continue;
