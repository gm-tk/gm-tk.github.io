...
async function main() {
  ...
  eng.loadEngine();
  // SESSION 40 R447 DIAG: every [button]-family ELEMENT item that mentions "journal" — its text, blackAfter, link
  { const CC = globalThis.ContentConverter; const _orig = CC.ConvertPage; const cut = (s, n = 120) => String(s ?? "").replace(/\s+/g, " ").trim().slice(0, n);
    CC.ConvertPage = function (page, bundles, run, normaliser) {
      for (const it of (page.items || [])) {
        const p = it && it.type === "tag" ? it.parse?.primary : null;
        if (!p) continue;
        const blob = String(it.text ?? "") + " " + String(it.blackAfter ?? "");
        if (!/journal/i.test(blob)) continue;
        _l(`ITEM\t${run.moduleCode}\t${page.filename || page.pageId || "?"}\ttag=${p.tag}\tdir=${p.directive}\ttext=${cut(it.text)}\tafter=${cut(it.blackAfter)}\tlink=${cut(it.block?.links?.[0]?.target, 60)}`);
      }
      return _orig.call(this, page, bundles, run, normaliser);
    };
  }
