#!/usr/bin/env python3
"""Session 26 — BEFORE-position variant (what PRECEDES the block in its column) of the EXTENDED (from _s25_after.py): the r51 `row_breaks.after` rule ("content following a closed callout / activity / widget always starts a
...
meta = load_meta()
TARGETS = ("whakatauki", "alert", "alert.solid", "table", "img", "video", "WIDGET")
            ...
            for side, cols in (("gold", g), ("claude", c)):
                for kids in cols:
                    for idx, kk in enumerate(kids):
                        if kk in TARGETS:
                            nxt = kids[idx - 1] if idx > 0 else None
                            agg[k][side][kk]["FIRST in column" if nxt is None else f"after {nxt}"] += 1
...
for k in sorted(agg, key=lambda k: (0 if k == "ALL" else 1 if k.startswith("template=") else 2, -agg[k]["pages"])):
    ...
    for kk in TARGETS:
        ...
        fl = sum(v for x, v in A["gold"][kk].items() if x != "FIRST in column")
        cfl = sum(v for x, v in A["claude"][kk].items() if x != "FIRST in column")
        flag = "  <== CANDIDATE" if (fl/max(gt,1) >= 0.60 and cfl/max(ct,1) <= 0.40 and len(pages_follow[(k, kk)]) >= 20 and len(mods_follow[(k, kk)]) >= 10) else ""
        print(f"  {kk:13s} gold   n={gt:5d} preceded-by-content {fl/max(gt,1):.2f} (pages {len(pages_follow[(k, kk)])} / {len(pages_any[(k, kk)])}, mods {len(mods_follow[(k, kk)])}): {line(A['gold'][kk])}{flag}")
        print(f"  {'':13s} claude n={ct:5d} preceded-by-content {cfl/max(ct,1):.2f}: {line(A['claude'][kk])}")
