 ...
 *      node ../../outputs/_measure_r352_widgetloss.cjs --merge      → _r354_tagwords.json
...
const SHARD = (k) => path.join(OUT, `_r354_tw_shard${k}.json`);
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r354_tagwords.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r354_tagwords.json"), all.length);
	return;
}
...
function runs(bundle) {
	...
	const add = (raw) => {
		const t = norm(String(raw ?? "").replace(RED, " ").replace(/\[[^\]]*\]/g, " ").replace(/https?:\/\/\S+/g, " "));
		if (t.split(" ").filter(Boolean).length >= 3) out.push(t);
	};
	add(m0.blackAfter); add(m0.text);
	return [...new Set(out)];
}
