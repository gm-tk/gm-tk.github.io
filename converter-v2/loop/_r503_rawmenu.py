#!/usr/bin/env python3
"""ROUND 503 copy of _r460_rawmenu.py (OFF = outputs/_r503_off saved under BLLKPTABS_OFF=1, ON = the disk) — the MENU-ONLY evidence. The skeleton SCAFFOLD collapses the module menu's `tabs` to one WIDGET marker, so the
...
pages = [p.strip() for p in open(os.path.join(O, "_r503_ON_pages.txt")) if p.strip()]
...
for code in codes:
    for n, cp, hp in DA.pairs(code):
        ...
        on = cp; off = os.path.join(O, "_r503_off", code, nm)
        g, d, o = K._skel(hp, False), K._skel(off, False), K._skel(on, False)
