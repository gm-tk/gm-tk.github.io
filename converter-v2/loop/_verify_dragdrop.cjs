...
// Per module: widgets N (images M) / drags D / defect. Non-standard layouts (column / FIB / scatter) are counted, not checked
// (no builder ships them). Protected criterion: defect 0 (✓ at the recorded baseline / ✗ above it — the r348 form).
//
...
// Selftest (INJECT): with CV2_SELFTEST_INJECT the verifier rewrites the first built widget's first drag to option="99"
// (no matching drop) and puts loading="lazy" on its image — both must register as defects (DETECTION); the fixture
// module must build > 0 widgets (LIVENESS).
"use strict";
...
function check(w) {
	...
	if (layout !== "standard") return r;   // column / FIB / scatter are not built by any builder — counted only
...
(async () => {
	let totW = 0, totImg = 0, totDrags = 0, totDefect = 0, totAbove = 0, mods = 0, improved = false, injected = false;
	for (const mod of process.argv.slice(2)) {
		let pages;
		try { pages = await convertModule(mod); } catch (e) { console.log(`${mod}: ERROR ${e.message}`); continue; }
		mods++;
		let n = 0, nImg = 0, drags = 0, defect = 0; const notes = [];
		for (const p of pages) {
			for (let w of widgets(p.html)) {
				...
				const r = check(w); n++; if (r.images) nImg++; drags += r.drags;
		...
		totW += n; totImg += nImg; totDrags += drags; totDefect += defect;
		console.log(`${mod}: widgets ${n} (images ${nImg}); drags ${drags}; defect ${defect}${defect > base ? ` ✗ (above the recorded baseline ${base})` : defect ? ` ✓ (at the recorded baseline ${base})` : " ✓"}`);
		for (const l of notes.slice(0, 12)) console.log(l);
	}
	console.log(`TOTAL: ${totW} widget(s) across ${mods} module(s); images ${totImg}; drags ${totDrags}; defect ${totDefect}.`);
