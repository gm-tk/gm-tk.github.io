#!/usr/bin/env python3
"""ROUND 456 — §1e population split from _r456_prescore.log (per gold page: OLD = the r454 disk, NEW = the ON pages): pairs on
both sides (the pre-existing population), new pairs, lost pairs; every per-page dip listed. Run under WSL."""
import re, os
H = os.path.dirname(os.path.abspath(__file__))
rows = []
for l in open(os.path.join(H, "_r456_prescore.log"), encoding="utf-8"):
