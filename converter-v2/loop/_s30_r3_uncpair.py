#!/usr/bin/env python3
"""Session 30 Round 3 PICK — pair every unclassified one-table bundle (_s30_r3_uncdump.tsv) with the gold: find the gold page of the
...
first cell; tally per (table width, red cells?, url cells?) → gold widget. Output: _s30_r3_uncpair.out"""
...
def main():
    rows = [l.rstrip("\n").split("\t") for l in open(os.path.join(HERE, "_s30_r3_uncdump.tsv"), encoding="utf-8") if "\t" in l]
    ...
    L = [f"unclassified one-table bundles: {len(rows)}", "by (width, red?, url?) → the gold widget holding the first data cell (n >= 15):"]
    ...
    for k, v in list(ex.items())[:40]:
        L.append(f"  {k}: " + " | ".join(v))
    txt = "\n".join(L)
    open(os.path.join(HERE, "_s30_r3_uncpair.out"), "w", encoding="utf-8").write(txt + "\n")
    print(txt[:9000])
main()
