#!/usr/bin/env python3
"""ROUND 486 — the companion numbers for the dipping pages (LOOP §3 step 6): the position-free overlap (multiset of skeleton line
roles, depth ignored, gold ∩ Claude) and the count of quote-form lines, OLD (disk) vs NEW (outputs/_r486_on). WSL, from outputs/:
...
for arg in sys.argv[1:]:
    ...
    n, _ = dm.page_lines(os.path.join(OUT, "_r486_on", code, page))
    G = roles(g); O = roles(o); N = roles(n)
    ov = lambda C: sum((G & C).values())
    q = lambda C: {k: v for k, v in C.items() if "quote" in k}
