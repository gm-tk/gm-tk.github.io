 ...
 *      node ../../outputs/_measure_r352_widgetloss.cjs --merge      → _s43_family.json
...
const SHARD = (k) => path.join(OUT, `_s43_fam_shard${k}.json`);
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_s43_family.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_s43_family.json"), all.length);
	return;
}
...
async function main() {
	...
	InteractiveBuilder.Build = function (args) {
		...
		try { if (b && b.type) pending.push({ type: b.type, code: curMod, page: curPage, index: b.index, runs: runs(b), built: out != null }); }
	...
	for (const code of list) {
		...
		try {
			...
			for (const p of pending) {
				...
				recs.push({ type: p.type, code: p.code, page: p.page, index: p.index, built: p.built, runs: p.runs.length, lost: lost.length, lostText: lost.slice(0, 4).map((x) => x.slice(0, 70)) });
			}
		} catch (e) { errs++; lastErr = String(e && e.stack || e).slice(0, 300); }
	}
