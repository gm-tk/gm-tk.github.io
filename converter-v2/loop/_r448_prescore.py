#!/usr/bin/env python3
"""ROUND 448 (session 40 Round 2 — D13-4, the answer-key carry-through) — pre-score the ON pages with the gate's own pairing and match().
For each changed module (_r444_ON_modules.txt): pairs() on the disk (the r447 state), then each pair's SCAFFOLD with the disk page (OLD) and
with the ON page saved by `_r448_probe_run.sh SAVE` (NEW). Prints every mover, the up / down counts, the pp-sum
...
codes = [c.strip() for c in open(os.path.join(O, "_r448_ON_modules.txt")) if c.strip()]
rows = []
for code in codes:
    for n, cp, hp in pairs(code):
        onp = os.path.join(O, "_r448_on", code, os.path.basename(cp))
