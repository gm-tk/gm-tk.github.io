#!/usr/bin/env python3
"""_r340_wordloss.py — ROUND 340 proof: over the affected pages, the ON output (outputs/_r340_on/<code>/<page>) must lose no
...
OUT = os.path.join(HERE, "..", "..", "01-Claude_Modules_"); OFF = os.path.join(HERE, "_r340_off")
AFF = [l.strip() for l in open(os.path.join(HERE, "_r340_affected.txt")) if l.strip()]
...
for c in AFF:
    d = os.path.join(OFF, c)
    if not os.path.isdir(d): continue
    for f in sorted(os.listdir(d)):
        a = open(os.path.join(d, f), encoding="utf-8").read(); b = open(os.path.join(_corpus.mdir(OUT, c), f), encoding="utf-8").read()
        if a == b: continue
