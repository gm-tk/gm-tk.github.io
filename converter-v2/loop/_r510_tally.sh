...
cd "$(dirname "$0")"
declare -A B A
for p in $(grep '\.html$' _r510_ON_pages.txt); do
  ...
  for t in $(grep -o 'INTERACTIVE (un-built) #[0-9]*: [A-Za-z]*' _r510_on/$p </dev/null | sed 's/.*: //'); do A[box:$t]=$(( ${A[box:$t]:-0} + 1 )); done
  ...
  for t in $(grep -o '<div class="\(dragAndDrop\|multiChoiceQuiz\|reorder\|radioQuiz\|dropQuiz\|typing\|selectionBox\|memoryGame\|wordFind\)[ "]' _r510_on/$p </dev/null | sed 's/<div class="//; s/[ "]$//'); do A[built:$t]=$(( ${A[built:$t]:-0} + 1 )); done
done
