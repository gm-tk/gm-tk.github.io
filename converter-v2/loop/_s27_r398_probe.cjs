/** _s27_r398_probe.cjs — ROUND 398 in-memory A/B probe (OFF = --pre <icon_rule json> + VIDEOICONWIDGET_OFF=1; ON = the live data). Converts each module exactly as batch_convert.cjs
 ...
 *    VIDEOICONWIDGET_OFF=1 STUB_OEMBED=1 node --require ./_deflate_raw_polyfill.cjs ../../outputs/_s27_r398_probe.cjs CODES…   (expect: all identical)
 *    STUB_OEMBED=1 node --require ./_deflate_raw_polyfill.cjs ../../outputs/_s27_r398_probe.cjs CODES…                     (the round's diffs)
...
async function main() {
  ...
  if (preFile) {   // ROUND 398 OFF leg: the committed (pre-round) registry, no widget_embedded block
    DataService.Data.EmitTemplates.video.icon_rule = JSON.parse(fs.readFileSync(preFile, "utf8"));
  }
