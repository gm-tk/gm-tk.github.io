...
for (const m of mods) {
	...
	let mq = 0, mi = 0, mh = 0, mt = 0; const gtext = " " + fs.readdirSync(gd).filter((x) => x.endsWith(".html")).map((f) => norm(fs.readFileSync(path.join(gd, f), "utf8").replace(/<!--[sS]*?-->/g, "").replace(/<[^>]+>/g, " "))).join(" ") + " ";
	for (const f of fs.readdirSync(cd).filter((x) => x.endsWith(".html"))) {
		...
		for (const q of h.matchAll(/<div class="typing[^"]*" layout="standard">\s*<div class="table-responsive">([\s\S]*?)<\/table>/g)) {
			...
			for (const a of ans) { mi++; if (gold.has(norm(a))) mh++; else if (gtext.includes(" " + norm(a) + " ")) mt++; }
		}
	}
	if (!mq) continue;
	quizzes += mq; inputs += mi; hit += mh; if (!gold.size) noGold += mi;
	console.log(`${m}: ${mq} table quiz(zes), ${mi} inputs, ${mh} = a gold answer, +${mt} in gold text${gold.size ? "" : " (module has NO gold typing input)"}`);
}
