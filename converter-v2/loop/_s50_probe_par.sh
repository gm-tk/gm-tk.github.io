#!/usr/bin/env bash
# Session 50 — the in-memory probe (_r448_probe.cjs) over a code list in 4 parallel shards (WSL).
...
for s in 00 01 02 03; do
  ( env "$@" STUB_OEMBED=1 timeout 1500 node --require ./_deflate_raw_polyfill.cjs $O/_r448_probe.cjs ${PROBE_QUIET:+--quiet} ${PROBE_SAVE:+--save $PROBE_SAVE} $(cat $O/_${TAG}_codes_$s) > $O/_${TAG}_$s.log 2>&1 ) &
done
wait
