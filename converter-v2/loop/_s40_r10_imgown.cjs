/** SESSION 40 ROUND 10 (24 Sept 2026) — the IMAGE OWN-TEXT RECORDER (the r278 technique): a patched copy of ContentConverter.js
...
async function main() {
  ...
  eng.loadEngine();
  const src = fs.readFileSync(path.join(eng.APP, "MediaBuilder.js"), "utf8");
  ...
  const tmp = path.join(__dirname, `_s40_r10_MB_patched_${process.pid}.cjs`);
