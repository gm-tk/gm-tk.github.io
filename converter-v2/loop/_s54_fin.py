#!/usr/bin/env python3
"""_s53_fin.py — session 54 (a copy of _s53_fin.py, itself a copy of _s52_fin.py)'s shared (a copy of _s49_fin.py; the in-flight and no-round anchors widened) FINALISE helper (WSL). One call per shipped engine round:
...
def finalise(*, N, old_build, new_build, entry, config_comment, og9=None, og11=None, og14, gb_note,
             ...
             before_them_prev_first=None, extra_remove=(), replace=()):
    """extra_remove: line prefixes (each unique, above the Round log) moved verbatim to the archive and deleted;
    replace: (old, new) pairs applied to LOOP_STATE.md, each old asserted unique."""
    ...
    xr = [find(p) for p in extra_remove]
    assert all(i < rl for i in xr), "extra_remove lines must sit above the Round log"
    xr_text = "\n".join(L[i] for i in xr)
    L.insert(rl + 1, roundlog)
    for i in sorted([others[0], bi] + xr, reverse=True):
        del L[i]
    if xr_text:
        archive_extra = archive_extra + "\n- **Removed from Position at the finalise (verbatim):**\n" + xr_text + "\n"
    io.open(A, "a", encoding="utf-8", newline="\n").write(
        f"\n## Position — Before it r{bprev.group(1)} + the prior no-round line (verbatim, s54 r{N})\n\n" + before_it + "\n" + prior_nr + "\n"
        f"\n## Session 54 — r{N} PICK (the in-flight marker, verbatim) + what shipped\n\n" + marker + "\n" + archive_extra + "\n")
    body = "\n".join(L)
    for old, new in replace:
        one(body, old, "replace")
        body = body.replace(old, new, 1)
    wr(S, body); print("LOOP_STATE", len(ss.encode("utf-8")), "->", os.path.getsize(S))
