"""_r342_leaks.py — list every literal-[tag] leak the protected gate counts (same detector as _structural_defect_audit.py),
...
for code in sorted(per,key=lambda c:-len(per[c])):
    ...
    for f,t,ctx in per[code]:
        ...
        seen[k]+=1
        print("   ",f,"|",t,"|",ctx[:150])
json.dump({c:per[c] for c in per},open(os.path.join(HERE,'_r342_leaks.json'),'w'),indent=1)
