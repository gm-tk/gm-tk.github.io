...
async function main() {
	...
	InteractiveBuilder.Build = function (args) {
		...
		if (b?.type === (process.env.DUMP_TYPE || "flipCard") && (!want || curPage.includes(want))) {
			_l(`=== ${code} page ${curPage} #${b.index} tables ${(b.tables ?? []).length}`); for (const m of [...(b.openerItems ?? []), ...(b.memberItems ?? [])]) _l("  ITEM " + (m?.type) + " " + JSON.stringify(m?.type === "tag" ? { text: m.text, after: m.blackAfter } : (m?.text ?? m?.block?.rows ?? "")).slice(0, 260));
