/** _measure_r351_ddprose.cjs — ROUND 351 PICK measurement (the D10-3 dragAndDrop kickoff, shapes 2+): the MEMBER ITEMS of every
 *  dragAndDrop bundle that has NO table or 2+ tables (the r350 census only dumped the tables).
 ...
 *      node ../../outputs/_measure_r350_ddcolumn.cjs --merge      → _r351_ddprose.json
...
const SHARD = (k) => path.join(OUT, `_r351_pr_shard${k}.json`);
const clean = (s) => String(s ?? "").replace(/\s+/g, " ").trim();
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r351_ddprose.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r351_ddprose.json"), all.length);
	return;
}
...
async function main() {
	...
	InteractiveBuilder.Build = function (args) {
		...
		try {
			seen++;
			if (b?.type === "dragAndDrop" && (b.tables ?? []).length !== 1) {
