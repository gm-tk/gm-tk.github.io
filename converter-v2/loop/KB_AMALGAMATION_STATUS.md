...
| 23 | Lesson-page menu labels normalised to `<h5>` (We are learning: / I can: / You will show your understanding by:) | pre-ledger | lesson pages with a menu | **AUTHORISED — Chris D10-9 (2026-09-16): apply everywhere, overview tab included, normalised wording by year level, no section title above; queued (FULL regeneration); the `<p>` residue is 206 labels / 176 lesson pages + ~296 overview pages (`_r341_menulabels.log`).** Was: **LIVE** — 216 / 247 / 260 Claude pages carry the three labels (gold 408 / 333 / 357; the difference is menu detection, tracked by the menu rounds) | MenuBuilder |
...
| — | 05D "Tables" — `table table-bordered` default; `table tableFixed` (no bordered) for a two-column comparison table; 06 §6's `table noHover tableFixed` superseded | **AUTHORISED — Chris D10-5 (2026-09-16), queued item 5**; 1,063 tables / 518 pages at r331 (re-measure) | skeleton-visible (named override on the Standard tie 0.51; gains Inquiry 0.72 / Fundamentals 0.69 / Bilingual 0.86; ≈ −0.01pp net) | the table's column count + a contrast-lexicon header pair (measured first) | FULL regeneration; KB DELTA: 06 §6 lines 405–411 → reconcile to 05D |
| — | 05A "MathJax / Equations" — Word (OMML) equations extracted and shipped as **MathML** (the gold's `<math xmlns>` form; LaTeX does not render in MTK — the V1.5 2026-08-26 test) + `body.mathJax` | **AUTHORISED — Chris D10-7 (2026-09-16), queued item 4**; 12 WTs / 329 equations / ~80 pages (gold 2,052 `<math>` on 94 pages) | skeleton-visible (up) | the docx `m:oMath` / `m:oMathPara` nodes | scoped to the 12 modules; verifier: OMML count == `<math>` count; KB DELTA: 05A lines 217–223 → MathML |
...
| — | (not a KB row — a house rule the KB lacks) a `\|`-separated bilingual LESSON title in a Standard-template module ships English first (MTK stays Māori first, 07D rule 7) | **AUTHORISED — Chris D10-2 (2026-09-16), queued item 3**; 4 pages (2 gain the gold's order, 2 become named overrides) | skeleton-neutral-to-a-hair | the r316 `reo_detect "macron"` discriminator | scoped; KB DELTA: 00G c79 / 01A to gain the sentence |
| — | 03B–03F + 15 — the un-built writer-tagged interactives BUILT by the loop, one widget type per kickoff, largest first (dragAndDrop 822 → clickDrop 324 → accordion 255 → carousel 239 → flipCard 215 → selfCheck 153 → modal 153 → tabs 75 → slider 47 → infoTrigger 37 → shapeHover 27 → hint 17 → hintSlider 12) | **AUTHORISED — Chris D10-3 (2026-09-16), queued item 7 (after the structure rounds)** | invisible to the skeleton by design — judged on each widget's own verifier (A1); the plateau "moved" test = the dashboard's *Still a box* count | the writer's tag + the r286 decline instrument's shape families (≥ 20 sites each) | family regeneration per type; never half-build |
