...
| 47 | Drop a body heading identical to the lesson `<h1>` (ignoring `Lesson N`) | pre-ledger | lesson pages | **AUTHORISED — Chris D10-1 (2026-09-16): c47 in full — the human's SSOG101 form (number top-right, own title as h1, no duplicate body heading; activity-box titles kept); queued (scoped).** Was: **PARTIAL** — Claude keeps a `Lesson N`-prefixed body heading on 89 pages (gold 18) | ContentConverter heading drop / relevel; skeleton-visible |
...
| 7 | c47 / c95 `Lesson N` body-heading strip / drop | **AUTHORISED — Chris D10-1 (2026-09-16), queued item 2**; 77 pages / 54 modules at r320 (re-measure) | skeleton-visible (named override on ≈ 6 gold-kept pages) | the heading's own text vs the `<h1>` (c47 test) | + the title's stray `*` / `**` markers as a measured sibling |
