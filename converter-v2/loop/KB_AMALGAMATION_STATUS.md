...
| 43 | Activity ending in a dropbox/portfolio button carries the `dropbox` wrapper modifier (BLL carve-out: never) | pre-ledger; CL-0021 | every dropbox activity outside BLL (187 WT modules mention a dropbox) | **PARTIAL → QUEUE #1** — a rule EXISTS: round 305's `body_region.activity_dropbox_postpass` (`ACTDROPBOX_OFF`) marks an activity when the writer's own upload BUTTON sits inside it (0.91 precision, 17% recall at r305), and round 308's `dbxbutton` builds the gold's "Upload to dropbox" button from dropbox bundles. Yet Claude today: 607 activities end in such a button and **208 non-BLL ones lack the modifier** (XDLS 63 · XMES 32 · XTAS 22 · TWHA 10 · ANZH/ENGI/ENGS 7 · HIS 6 …); gold applies it to 717 of 816 non-BLL such activities (88%) and withholds it on 480 of 512 BLL ones (94% — the carve-out is real, and r308 recorded the buttonD blindness as deliberately correct). Claude already honours the BLL carve-out (292 of 292 without) | `ActivitiesBuilder.activityDropboxPostpass` + `Emit_Templates.body_region.activity_dropbox_postpass`; **skeleton-visible** (`div.activity.dropbox`) |
...
| 45 | Acks wrapper `acks acksTemplate` (+`acksAI`) | pre-ledger; CL-0045; CL-0090 | every module (413 overview pages) | **NOT CAPTURED** — `acksTemplate` on 19 Claude pages (only with `acksAI`); the other 394 ship bare `acks` | AcksBuilder / `Acks_Formats.json` — see row 90 |
...
| 55 | "Go to dropbox" / "Go to portfolio" labels; BLL/LS/HPE "Upload to Dropbox" | CL-0004; CL-0023 | dropbox modules | **LIVE** — but **420 Claude buttons end in a full stop** (`Upload to dropbox.` 137, `Learning journal.` 37, `Check answers.` 27 …; gold 3) — a text defect to strip | buttons emitter; gate-neutral |
...
| 65 | `[MTKquiz]` = numbered activity shell: title h3, instructions, To Do note, blank-href "Go to quiz"; **never the quiz content** | CL-0038; CL-0082 | 12 WT modules | **PARTIAL** — the button is LIVE (r232, `MTKQUIZ_OFF`; 78 Claude buttons); the content omission is **NOT CAPTURED** (56 of 80 Claude shells still carry `<ol>/<ul>`/quiz markup; gold 157 of 199 — pre-rule) | `interactive_builders.mtk_quiz`; skeleton-visible |
...
| 79 | Lesson page `<h1><span>` = the lesson's own title (fixed source order; `Lesson N` stripped; module title only as a disclosed fallback; a second h1 only for the lesson's own bilingual pair) | CL-0069/0076 | 1534 paired lesson pages | **PARTIAL → QUEUE #3** — Claude matches gold on 882; falls back to the module title on 141 (gold itself does on 233); gold ships the lesson's bilingual pair on 237 pages vs Claude 10; sampled mismatches: the code-prefixed joined form (`PNR101 Number 1 \| Te tau 1`), `Lesson One` word-numbers kept, trailing full stops, sub-page fallbacks (ANZH304 0.1–0.6) | ContentConverter header titles; **skeleton-visible** (h1 count) |
...
| 83 | No `loading="lazy"` inside moving widgets (banner, carousel, D&D, clickDrop, flipCard, memoryGame, sketcher) | CL-0083 | every image inside those hosts | **NOT CAPTURED → QUEUE #5** — 1121 of 1215 Claude images inside moving hosts carry `loading="lazy"` on ≈ 243 pages (gold 3472 of 15322) | r240 `img_alt_lazy` adds it everywhere; gate-neutral (attribute not in KEEP_ATTR) |
...
| 89 | `learningSupport` on `<html>` for every X-prefixed code | CL-0089 (locked admin) | 36 modules / 228 Claude pages | **NOT CAPTURED → QUEUE #4** — 0 Claude pages carry it (gold 72 of 250 — the KB outranks) | PageAssembler shell; gate-neutral (the `<html>` tag is outside the skeleton) |
| 90 | `acksTemplate`/`acksAI` generate their statements — never type the apology, AI or copyright lines (`currentYear` too); keep only "All other images ©" | CL-0090 (locked admin) | every module (413 overview pages) | **NOT CAPTURED → QUEUE #2** — Claude types the statements on 394 pages (gold 443 — pre-rule); r241 already omits them for the `acksAI` variant | `Acks_Formats.standing_items.template_variant_omit` (`ACKSBOILER_OFF`) — extend to every block; corpus-wide; **skeleton-visible** (two `acksLesson` divs fewer) |
...
|---|---|---|---|---|---|
| 1 | c43 dropbox wrapper modifier | 208 non-BLL activities lacking it (XDLS 63 · XMES 32 · XTAS 22 · TWHA 10 …) | skeleton-visible (`div.activity.dropbox`) | the activity's own closing button | gold (88%) and KB agree; the BLL carve-out already holds in Claude |
| 2 | c90 + c45 acks wrapper class + no typed statements | 413 overview pages (corpus-wide) | skeleton-visible as a NAMED OVERRIDE (gold types the statements on 443 pages) | unconditional | locked admin decision; extend the r241 `template_variant_omit` to every block; full regeneration (acks = corpus-wide) |
