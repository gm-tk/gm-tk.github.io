#!/usr/bin/env python3
"""Session 29 Round 10 instrument (the s27-r1 census re-run on the r418 corpus) — THE POSITION-FREE LABEL CENSUS.
...
reported (an upper bound on the gate's difflib ratio; the gap = the order loss).
  wsl: python3 _s29_r10_labelcensus.py  →  _s29_r10_labelcensus.out / .json"""
...
open(os.path.join(OUTPUTS, "_s29_r10_labelcensus.out"), "w", encoding="utf-8").write("\n".join(out) + "\n")
json.dump({"pairs": npairs, "pf_mean": sum(pf_ratios)/max(1,len(pf_ratios)),
           ...
           "per_page": per_page},
          open(os.path.join(OUTPUTS, "_s29_r10_labelcensus.json"), "w"), indent=0)
