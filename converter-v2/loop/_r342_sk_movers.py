import json, sys
a=json.load(open('_r341_sk_final.json'))['per_page']; b=json.load(open('_r342_sk_final.json'))['per_page']
...
def raw(v):
    ...
    return None
aff=set(l.strip() for l in open('_r342_s11_affected.txt') if l.strip())
mv=[]
for k in A:
    if k in B:
        x,y=sc(A[k]),sc(B[k])
