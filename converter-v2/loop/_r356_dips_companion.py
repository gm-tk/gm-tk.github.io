#!/usr/bin/env python3
"""_r356_dips_companion.py — ROUND 356 proof (the r352 script re-pointed: OFF pages = the r354 ON probe = the pre-r356 disk): the LOOP §3 step-6 companion numbers for every skeleton dip page. For each page:
the skeleton SequenceMatcher ratio (the gate) BEFORE (the OFF engine's page, saved in outputs/_r354_on) and AFTER (the
...
pre = json.load(open(os.path.join(OUT, "_r355_sk_final.json")))
post = json.load(open(os.path.join(OUT, "_r356_sk_final.json")))
