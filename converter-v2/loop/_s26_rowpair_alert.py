...
print("(A) per template+subject — alert kinds, every group")
for (k, kd), cnt in sorted(A.items()):
    if not kd.startswith("alert") or not k.startswith("tmpl+subj="): continue
    dis = cnt.get("GOLD-FLOWS (Claude broke)", 0); ag = cnt.get("gold-breaks (Claude right)", 0)
    if dis + ag == 0: continue
    print(f"   {k:48s} {kd:14s} flow {dis:3d} / break {ag:3d} = {dis/(dis+ag):.2f}")
print("pages/modules by kind (ALL):", {k: (len(Apages[k]), len(Amods[k])) for k in ("alert", "alert.solid", "alert.top", "alertActivity")})
print("(B) div kind examples:", EXB.get("div"))
