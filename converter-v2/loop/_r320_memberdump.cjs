/** _r320_memberdump.cjs — ROUND 314 DIAGNOSTIC. For a module, dump every page's bundles with the
...
async function main() {
  ...
  for (const page of pages) {
    ...
    for (const b of bundles) {
      if (b.type === "dropDown") {
        _l(`  DROPDOWN bundle openerItems=${(b.openerItems??[]).length} memberItems=${(b.memberItems??[]).length}`);
        for (const m of (b.openerItems??[])) _l(`     O idx=${items.indexOf(m)} type=${m.type} tag=${m.parse?.primary?.tag??"-"} «${clip(String(m.text??""),70)}» after=«${clip(String(m.blackAfter??""),40)}»`);
        for (const m of (b.memberItems??[])) _l(`     M idx=${items.indexOf(m)} type=${m.type} tag=${m.parse?.primary?.tag??"-"} «${clip(String(m.text??""),70)}» after=«${clip(String(m.blackAfter??""),40)}»`);
      }
