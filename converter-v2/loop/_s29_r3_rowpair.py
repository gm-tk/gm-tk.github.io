#!/usr/bin/env python3
"""Session 29 Round 3 PICK — the s26 PAIRED ROW-BREAK CENSUS scoped to argv prefixes (+ a prefix= key) for every block type, both directions (the _s26_whpair.py idea generalised).
...
PFX = [a for a in sys.argv[1:] if not a.startswith("-")]
for code in sorted(fam):
    pre = re.match(r"[A-Z]+", code).group(0)
    if PFX and pre not in PFX: continue
    tf = fam[code]; subject = (meta.get(code, {}) or {}).get("subject") or "None"
    keys = ("ALL", f"template={tf}", f"tmpl+subj={tf}/{subject}", f"prefix={pre}")
