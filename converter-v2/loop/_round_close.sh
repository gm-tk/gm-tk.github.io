...
cd "$ROOT"
# 1. mirror
for f in _verify_dragdrop.cjs _selftest_core.cjs run_all_gates.sh gate_baseline.json _corpus.py batch_convert.cjs _skeleton_compare.py _diff_miner.py; do cp "$T/$f" "$L/$f"; done
cp LOOP_STATE.md KB_AMALGAMATION_STATUS.md LOOP__Autonomous_Rounds.md DIFF_QUEUE.md "$L/"
...
bad=0; cnt=0
for f in $(cd "$L" && ls --indicator-style=none _${R}* _measure_${R}* _verify_dragdrop.cjs _selftest_core.cjs run_all_gates.sh gate_baseline.json _corpus.py batch_convert.cjs _skeleton_compare.py _diff_miner.py LOOP_STATE.md KB_AMALGAMATION_STATUS.md LOOP__Autonomous_Rounds.md DIFF_QUEUE.md COVERAGE_DASHBOARD.md 2>/dev/null); do
