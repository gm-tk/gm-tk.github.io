#!/usr/bin/env python3
"""ROUND 446 (session 39 Round 8 — D13-9 part 2, the bubble placeholder) — pre-score the ON pages with the gate's own pairing and match().
...
with the ON page saved by `_r446_probe_run.sh SAVE` (NEW). Prints every mover, the up / down counts, the pp-sum
...
codes = [c.strip() for c in open(os.path.join(O, "_r446_ON_modules.txt")) if c.strip()]
rows = []
for code in codes:
    for n, cp, hp in pairs(code):
        onp = os.path.join(O, "_r446_on", code, os.path.basename(cp))
