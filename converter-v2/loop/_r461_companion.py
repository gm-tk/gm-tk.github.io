...
and the matched-line count (difflib's matching blocks), disk (r460) vs ON (outputs/_r461_on). Pairs = the gate's own pairing
...
pages = set(p.strip() for p in open(os.path.join(O, "_r461_ON_pages.txt")) if p.strip())
...
for code in codes:
    for _, cp, hp in DA.pairs(code):
        ...
        on = os.path.join(O, "_r461_on", code, nm)
