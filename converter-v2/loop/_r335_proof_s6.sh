#!/usr/bin/env bash
# ROUND 335 (session 6) — the proof suite: fast-loop decomposition, full protected gates,
...
python3 _fastloop_diff.py $(xargs < $O/_r335_affected.txt) > $O/_r335_fastloop_s6.log 2>&1; echo "  rc=$?"
echo "[$(date +%T)] full gate suite"
bash run_all_gates.sh > $O/_r335_gates_s6.log 2>&1; echo "  rc=$?"
echo "[$(date +%T)] fresh skeleton --json"
python3 _skeleton_compare.py --json $O/_r335_sk_final.json > $O/_r335_sk_full.log 2>&1; echo "  rc=$?"
...
} > $O/_r335_selftests_s6.log 2>&1; echo "  rc=$?"
echo "[$(date +%T)] DONE"
