/** _measure_r351_ddmembers.cjs — ROUND 351 measurement: the MEMBER ITEMS of every BUILT dragAndDrop bundle (does a built widget
 *  replace member prose the r69 / r350 forms never checked?).
 ...
 *      node ../../outputs/_measure_r350_ddcolumn.cjs --merge      → _r351_ddmembers.json
...
const SHARD = (k) => path.join(OUT, `_r351_mb_shard${k}.json`);
const clean = (s) => String(s ?? "").replace(/\s+/g, " ").trim();
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r351_ddmembers.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r351_ddmembers.json"), all.length);
	return;
}
