...
async function main() {
  ...
  eng.loadEngine();
  // ROUND 446 DIAG: log every speechBubble bundle the builder sees and what it returns
  { const _ib = globalThis.InteractiveBuilder; const _orig = _ib.Build; const _cut = (s, n = 90) => String(s ?? "").replace(/s+/g, " ").slice(0, n);
    _ib.Build = function (args) { const r = _orig.call(this, args); const b = args && args.bundle; if (b && b.type === "speechBubble") {
      const t = (b.tables || []).map((tb) => (tb.rows || []).map((row) => "[" + row.map((c) => _cut(c, 60)).join(" || ") + "]").join(" ; "));
      const m = (b.media || []).map((x) => _cut(x && (x.target || x.text), 70));
      const mem = (b.memberItems || b.members || []).length;
      _l(`BUBBLE ${args.run && args.run.moduleCode} #${b.number ?? b.index ?? "?"} mod=${_cut(b.modifier, 30)} tables=${(b.tables || []).length} members=${mem} media=${JSON.stringify(m)} -> ${r ? (/<img/.test(r) ? "BUILT avatar" : "BUILT text-only") : "NULL"}
     tables: ${t.join(" | ")}
     text: ${_cut(b.text || b.content || "", 120)}`); } return r; }; }
