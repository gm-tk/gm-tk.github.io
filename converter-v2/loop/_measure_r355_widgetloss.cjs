 ...
 *      node ../../outputs/_measure_r352_widgetloss.cjs --merge      → _r355_widgetloss.json
...
const SHARD = (k) => path.join(OUT, `_r355_wl_shard${k}.json`);
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r355_widgetloss.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r355_widgetloss.json"), all.length);
	return;
}
