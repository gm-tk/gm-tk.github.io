#!/usr/bin/env python3
"""ROUND 491: OLD = the toggles-OFF render (outputs/_r491_off), NEW = the disk — the companion numbers for the dipping pages (LOOP §3 step 6): the position-free overlap (multiset of skeleton line
roles, depth ignored, gold ∩ Claude) and the count of quote-form lines, OLD (disk) vs NEW (outputs/_r491_off). WSL, from outputs/:
...
for arg in sys.argv[1:]:
    ...
    n, _ = dm.page_lines(os.path.join(OUT, "_r491_off", code, page))
    G = roles(g); O = roles(o); N = roles(n)
    ov = lambda C: sum((G & C).values())
    q = lambda C: {k: v for k, v in C.items() if "carousel" in k or "WIDGET" in k}
    print(f"{code}/{page}: gold {len(g)} lines | position-free overlap OLD(r490) {ov(N)} / {len(n)}  NEW(r491) {ov(O)} / {len(o)}  "
