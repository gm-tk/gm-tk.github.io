// Session 50 Round 1 (r512) — every red span whose parse CHANGES when the generic-interactive-bracket rule is on:
	...
	for (const f of fs.readdirSync(d).filter((x) => x.endsWith("_parsed.txt") && !/^[^+]* Media List_parsed\.txt$/.test(x))) {
		for (const ln of fs.readFileSync(path.join(d, f), "utf8").split("\n")) {
			for (const x of ln.matchAll(/\u{1f534}\[RED TEXT\]([\s\S]*?)\[\/RED TEXT\]\u{1f534}/gu)) {
				...
				process.env.IQEMBED_OFF = "1"; const off = n.Parse(red); delete process.env.IQEMBED_OFF;
...
_l(`spans changed by r512: ${tot} in ${mods.size} modules`);
