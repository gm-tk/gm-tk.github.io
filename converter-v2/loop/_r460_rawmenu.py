#!/usr/bin/env python3
"""ROUND 460 — the MENU-ONLY evidence. The skeleton SCAFFOLD collapses the module menu's `tabs` to one WIDGET marker, so the
...
pages = [p.strip() for p in open(os.path.join(O, "_r460_ON_pages.txt")) if p.strip()]
...
for code in codes:
    for n, cp, hp in DA.pairs(code):
        ...
        on = os.path.join(O, "_r460_on", code, nm)
