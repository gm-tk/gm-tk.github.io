/** _s51_r4_contlost.cjs — every red span carrying a CONTAINER_OPEN tag whose PRIMARY is a HEADING (h1-h6 / heading); was: every red span whose tags include the activity tag but whose PRIMARY is some
...
async function main() {
	...
	for (const code of process.argv.slice(2).filter((a) => !a.startsWith("--")).map((a) => a.trim()).filter(Boolean)) {
		...
		for (const it of items) {
			...
			if (!tags.some((t) => t.directive === "CONTAINER_OPEN") || !/^(h[1-6]|heading)$/.test(it.parse.primary?.tag ?? "")) continue;
