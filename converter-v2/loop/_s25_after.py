#!/usr/bin/env python3
"""Session 25 — the r51 `row_breaks.after` rule ("content following a closed callout / activity / widget always starts a
...
follows. python3 _s25_after.py [prefix-filter]"""
import os, sys, re
                ...
                for kids in cols:
                    for idx, kk in enumerate(kids):
                        if kk in TARGETS:
                            ...
                            if nxt is not None and side == "gold": pages_follow[(k, kk)].add(hp)
...
for k in sorted(agg, key=lambda k: (0 if k == "ALL" else 1 if k.startswith("template=") else 2, -agg[k]["pages"])):
    ...
    for kk in TARGETS:
        ...
        print(f"  {kk:9s} gold   n={gt:5d} followed-on {fl/max(gt,1):.2f} (pages {len(pages_follow[(k, kk)])}): {line(A['gold'][kk])}")
        print(f"  {'':9s} claude n={ct:5d}: {line(A['claude'][kk])}")
