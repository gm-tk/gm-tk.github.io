 ...
 *      node ../../outputs/_measure_r352_widgetloss.cjs --merge      → _r356_videos.json
...
const SHARD = (k) => path.join(OUT, `_r356_vid_shard${k}.json`);
if (process.argv.includes("--merge")) {
	const all = [];
	for (let k = 0; k < 64; k++) { if (fs.existsSync(SHARD(k))) all.push(...JSON.parse(fs.readFileSync(SHARD(k), "utf8"))); }
	fs.writeFileSync(path.join(OUT, "_r356_videos.json"), JSON.stringify(all));
	console.log("records ->", path.join(OUT, "_r356_videos.json"), all.length);
	return;
}
...
function runs(bundle) {
	// every YouTube video id the writer referenced anywhere in the bundle (tag lines, black lines, link targets, cells)
	const ids = new Set();
	const grab = (t) => { for (const m of String(t ?? "").matchAll(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([\w-]{11})/g)) ids.add(m[1]); };
	for (const m of [...(bundle.openerItems ?? []), ...(bundle.memberItems ?? [])]) {
		if (!m) continue;
		grab(m.text); grab(m.blackAfter);
		for (const l of (m.block?.links ?? [])) grab(l?.target);
		if (m.type === "table") for (const r of (m.block?.rows ?? [])) for (const c of (r ?? [])) grab(c);
	}
	for (const t of (bundle.tables ?? [])) for (const r of (t.rows ?? [])) for (const c of (r ?? [])) grab(c);
	return [...ids];
}
	...
	for (const code of list) {
		...
		try {
			...
			for (const o of run.outputs.filter((o) => /\.html$/.test(o.filename))) {
				...
				pageText[o.filename] = String(o.content ?? "");   // raw html: the id lives in the iframe src
			}
			const allText = Object.values(pageText).join(" | ");
			for (const p of pending) {
				const lost = p.runs.filter((r) => !allText.includes(r));   // an id absent from every page = a video the writer asked for that no widget or free-body embed carries
