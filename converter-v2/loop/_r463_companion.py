...
and the matched-line count (difflib's matching blocks), disk (r462) vs ON (outputs/_r463_on). Pairs = the gate's own pairing
...
pages = set(p.strip() for p in open(os.path.join(O, "_r463_ON_pages.txt")) if p.strip())
...
for code in codes:
    for _, cp, hp in DA.pairs(code):
        ...
        on = os.path.join(O, "_r463_on", code, nm)
